...
export const productAPI = {
   ...
   */
  getProductById: async (id: string, lang?: Locale) => {
    const params = new URLSearchParams();
    params.append('lang', lang || getCurrentLang());
   ...
   */
  getTopRated: async (lang = getCurrentLang()) => {
    const params = new URLSearchParams();
...
export const categoryAPI = {
   ...
   */
  getCategories: async (lang?: Locale) => {
...
export const couponAPI = {
   ...
   */
  getCoupons: async (pageNumber = 1, keyword = '', pageSize = 10, discountType?: 'percentage' | 'fixed', locale?: string) => {
    const params = new URLSearchParams();
   ...
   * Tính giảm giá
   */
  calculateDiscount: async (couponCode: string, orderAmount: number, orderCurrencyCode: string, products?: string[]) => {
...
export const reviewAPI = {
   ...
   */
  getProductReviews: async (productId: string, lang?: Locale) => {
    const endpoint = `/reviews/products/${productId}/reviews`;
    const finalEndpoint = lang ? `${endpoint}?lang=${lang}` : buildLocalizedUrl(endpoint);
    const currentLang = lang || getCurrentLang();
    return apiCall(`${endpoint}?lang=${currentLang}`);
  },
...
export interface CreateOrderData {
  ...
  currencyCode?: string; // PHASE 3: Currency code at order creation time (VND, USD, EUR, SEK)
}
...
export const orderAPI = {
   ...
   */
  getAllOrders: async (pageNumber?: number, lang?: Locale) => {
    const params = new URLSearchParams();
    if (pageNumber) params.append('pageNumber', pageNumber.toString());
    const endpoint = `/orders${params.toString() ? '?' + params.toString() : ''}`;
    const separator = params.toString() ? '&' : '?';
    const finalEndpoint = lang ? `${endpoint}${separator}lang=${lang}` : buildLocalizedUrl(endpoint);
    return apiCall(finalEndpoint);
  },
   ...
   */
  getDeletedOrders: async (pageNumber?: number, lang?: Locale) => {
    const params = new URLSearchParams();
    params.append('pageNumber', pageNumber?.toString() || '1');
    const endpoint = `/orders/deleted/list?${params.toString()}`;
    const finalEndpoint = lang ? `${endpoint}&lang=${lang}` : buildLocalizedUrl(endpoint);
    return apiCall(finalEndpoint);
  },
   ...
   */
  getMyOrders: async (lang?: Locale) => {
    const endpoint = `/orders/myorders`;
    const finalEndpoint = lang ? `${endpoint}?lang=${lang}` : buildLocalizedUrl(endpoint);
    return apiCall(finalEndpoint);
  },
   ...
   * @param lang - Ngôn ngữ để lấy thông tin sản phẩm
   */
  getOrderById: async (id: string, lang: Locale) => {
    const endpoint = `/orders/${id}`;
    return apiCall(`${endpoint}?lang=${lang}`);
  },

  /**
   * Lấy chi tiết đơn hàng (alias for getOrderById)
   */
  getOrder: async (id: string, lang: Locale) => {
    const endpoint = `/orders/${id}`;
    return apiCall(`${endpoint}?lang=${lang}`);
  },
...
export const customerAPI = {
  ...
  getCustomerById: async (id: string) => {
    return apiCall(buildLocalizedUrl(`/customers/${id}`));
  },
...
export const analyticsAPI = {
   ...
   * @param lang - Ngôn ngữ (mặc định: hiện tại)
   */
  getDashboardData: async (days = 30, lang = getCurrentLang(), currencyCode: string) => {
    const params = new URLSearchParams({
      days: days.toString(),
      lang,
   ...
   */
  getUnpaidOrders: async (limit = 20, days = 30, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
    params.append('days', days.toString());
    params.append('lang', lang);
   ...
   * @param lang - Ngôn ngữ (mặc định: hiện tại)
   */
  getInactiveCustomers: async (limit = 10, days = 90, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
    params.append('days', days.toString());
    params.append('lang', lang);
   ...
   */
  getLowInventoryProducts: async (limit = 10, page = 1, sort = 'countInStock', threshold = 10, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
   ...
   */
  getLowRatingProducts: async (limit = 10, page = 1, sort = 'rating', ratingThreshold = 3.0, minReviews = 1, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
    params.append('page', page.toString());
    params.append('sort', sort);
   ...
   */
  getPaidOrders: async (limit = 20, page = 1, sort = '-createdAt', days = 30, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
    params.append('page', page.toString());
    params.append('sort', sort);
   ...
   * @param lang - Ngôn ngữ (mặc định: hiện tại)
   */
  getUnusedCoupons: async (limit = 10, page = 1, sort = 'currentUses', maxUsageRatio = 0.5, lang = getCurrentLang()) => {
    const params = new URLSearchParams();
    params.append('limit', limit.toString());
    params.append('page', page.toString());
    params.append('sort', sort);
...
export const shippingAPI = {
   ...
   */
  calculateShipping: async (to: { districtId: number; wardCode: string }, weight: number, value?: number) => {
    const endpoint = '/shipping/calculate';
    const finalEndpoint = buildLocalizedUrl(endpoint);
    return apiCall(finalEndpoint, {
      method: 'POST',
      body: JSON.stringify({ to, weight, value }),
    });
  },
