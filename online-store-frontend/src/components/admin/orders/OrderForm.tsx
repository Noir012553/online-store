...
import { MapPin, Package, DollarSign, Plus, X, AlertCircle } from 'lucide-react';
...
export function OrderForm({ mode, orderId, onSuccess, onCancel }: OrderFormProps) {
  ...
  useEffect(() => {
    ...
    orderAPI.getSummary({
      cartItems: orderItems.map((item) => ({ productId: item.product, quantity: item.qty })),
  ...
  }, [currency.code, locale, orderItems]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiCall(`/orders?lang=${locale}&locale=${locale}`, {
        method: 'POST',
        body: JSON.stringify({
      ...
      <div className="space-y-6 rounded-lg border bg-white p-4 sm:p-6">
                ...
                >
                  ...
                  {products.map((product) => (
                    <option key={product._id} value={product._id}>
                      {product.name} ({t('in_stock', 'admin')}: {product.countInStock})
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="overflow-x-auto rounded-lg border">
              <table className="w-full min-w-[620px] text-sm">
                ...
                <tbody className="divide-y">
                  {orderItems.map((item) => {
                    const summaryItem = summary?.orderItems.find((candidate) => candidate.product === item.product);
                    const product = products.find((candidate) => candidate._id === item.product);
                    return (
                      <tr key={item.product}>
                        <td className="px-4 py-2">{summaryItem?.name || product?.name || t('product_fallback', 'admin')}</td>
                        <td className="px-4 py-2 text-center">{item.qty}</td>
                        <td className="px-4 py-2 text-right">
                          {summaryItem?.formattedPrice || '—'}
                        </td>
                        <td className="px-4 py-2 text-right font-medium text-red-600">
                          {summaryItem?.formattedLineTotal || '—'}
                        </td>
        ...
        {orderItems.length > 0 && (
          <div className="space-y-3 rounded-lg border border-blue-200 bg-blue-50 p-4">
            ...
            {isLoadingSummary ? (
              <p className="text-sm text-gray-600">{t('loading', 'common')}</p>
            ) : summary ? (
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-600">{t('subtotal', 'admin')}</span>
                  <span className="font-medium">{summary.formattedItemsPrice}</span>
                </div>
                {summary.shippingFee > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">{t('shipping_fee', 'admin')}</span>
                    <span className="font-medium">{summary.formattedShippingFee}</span>
                  </div>
                )}
                <div className="flex justify-between border-t border-blue-200 pt-2">
                  <span className="font-semibold text-gray-700">{t('total', 'admin')}</span>
                  <span className="text-lg font-bold text-red-600">{summary.formattedTotalPrice}</span>
                </div>
              </div>
            ) : null}
          </div>
