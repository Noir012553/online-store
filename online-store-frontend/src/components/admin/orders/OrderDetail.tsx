...
import { apiCall, orderAPI } from '../../../lib/api';
...
export function OrderDetail({ orderId }: OrderDetailProps) {
  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        setIsLoading(true);
        const order = await orderAPI.getOrderById(orderId, locale, locale, currencyCode);
        setOrder(order);
      } catch (error) {
  ...
  }, [orderId, locale, currencyCode, t, router]);
  ...
  const handleMarkAsDelivered = async () => {
    ...
    try {
      ...
      await apiCall(`/orders/${order._id}/deliver?${params.toString()}`, { method: 'PUT' });
