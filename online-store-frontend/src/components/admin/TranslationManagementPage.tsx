...
export function TranslationManagementPage() {
  ...
  const handleBulkTranslate = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_BASE}/api/translations/bulk-sync`, {
        method: 'POST',
        headers: {
      ...
      setMessage({
        type: 'error',
        text: `${t('translation_management_error_prefix', 'admin')} ${error instanceof Error ? error.message : 'Unknown error'}`,
      });
    } finally {
  ...
  const handleClearCache = async () => {
    if (!confirm(t('translation_management_clear_cache_confirm', 'admin'))) {
      return;
    }
            ...
            >
              ...
              {SUPPORTED_LANGUAGES.map((lang) => (
                <option key={lang} value={lang}>
                  {lang === 'vi' && t('translation_management_language_vietnamese_option', 'admin')}
                  {lang === 'en' && t('translation_management_language_english', 'admin')}
                </option>
              ))}
            </select>
          </div>
        ...
        <div>
          {SUPPORTED_LANGUAGES.map((lang) => (
            <span key={lang} className="language-badge">
              {lang === 'vi' && t('translation_management_language_vietnamese_badge', 'admin')}
              {lang === 'en' && t('translation_management_language_english_badge', 'admin')}
            </span>
          ))}
        </div>
      </div>
