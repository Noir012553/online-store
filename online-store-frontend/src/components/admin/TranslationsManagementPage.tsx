...
const TranslationsManagementPage = () => {
  ...
  const fetchTranslations = async (page = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/translations/admin/list?page=${page}&limit=${pagination.limit}&includeDeleted=${includeDeleted}`,
        {
          headers: {
  ...
  const handleUpdateKey = async (translationId: string) => {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(
        `${API_BASE}/api/translations/admin/${translationId}/key`,
        {
          method: 'PUT',
          headers: {
  ...
  const handleDeleteKey = async (translationId: string, keyName: string) => {
    if (!window.confirm(t('admin_translations_confirm_soft_delete', 'common'))) return;
  ...
  const handleSoftDelete = async (translationId: string) => {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(
        `${API_BASE}/api/translations/admin/${translationId}/soft`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleHardDelete = async (translationId: string) => {
    if (!window.confirm(t('admin_translations_confirm_hard_delete', 'common'))) return;

    try {
      const token = getAuthToken();
      const response = await fetch(
        `${API_BASE}/api/translations/admin/${translationId}/hard`,
        {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleRestore = async (translationId: string) => {
    try {
      const token = getAuthToken();
      const response = await fetch(
        `${API_BASE}/api/translations/admin/${translationId}/restore`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const getLangDisplay = (code: string) => {
    const langKeys: Record<string, string> = {
      vi: 'admin_translations_management_language_vi',
      en: 'admin_translations_management_language_en',
    };
    const key = langKeys[code];
    return key ? t(key, 'common') : code;
  };
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y">
                {translations.map((translation) => (
                  <React.Fragment key={translation._id}>
        ...
        {pagination.pages > 1 && (
          <div className="mt-6 flex items-center justify-between">
            <div className="text-sm text-gray-600">
              {t('admin_translations_page', 'common')} {pagination.page} {t('of', 'common') || 'of'}{' '}
              {pagination.pages}
            </div>
