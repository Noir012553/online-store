...
import { toDateInputValue, getCouponCount, CouponFormState, CouponRecord, MultiSelectOption } from './coupons/couponUtils';
          ...
          <DialogHeader>
            <DialogTitle>
              {deleteTarget?.action === 'hardDelete'
                ? t('admin_coupon_hard_delete_title', 'admin')
                : t('admin_coupon_delete_title', 'admin')}
            </DialogTitle>
            <DialogDescription>
              {deleteTarget?.action === 'hardDelete'
                ? t('admin_coupon_hard_delete_desc', 'admin')
                : t('admin_coupon_delete_desc', 'admin')}
            </DialogDescription>
          </DialogHeader>
          <div className="rounded-xl border bg-amber-50 p-4 text-sm text-amber-800">
            ...
            <div className="mt-1">
              {deleteTarget?.coupon.discountType === 'percentage'
                ...
                : deleteTarget
                  ? formatCouponValue(deleteTarget.coupon)
                  : ''}
            </div>
          </div>
          <DialogFooter>
