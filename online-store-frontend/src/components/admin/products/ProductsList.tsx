...
export function ProductsList({ discountMode = false }: ProductsListProps) {
  ...
  }, [currentPage, searchQuery, filterCategory, itemsPerPage, locale, currencyCode, t]);
  ...
  useEffect(() => {
    ...
    return () => {
      leaveAdminRoom();
      offEvent('product-created', handleProductCreated);
      offEvent('product-updated', handleProductUpdated);
      offEvent('product-deleted', handleProductDeleted);
      offEvent('product-restored', handleProductRestored);
    };
                  ...
                  <td className="px-4 py-4 sm:px-6 font-medium">
                    ...
                    {product.originalPrice && product.originalPrice > 0 && product.price && product.originalPrice > product.price && (
                      <div className="mt-1 flex items-center gap-2 text-xs">
                        <span className="line-through text-gray-500">{product.formattedOriginalPrice}</span>
