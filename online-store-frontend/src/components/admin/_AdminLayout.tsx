...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { t, loadNamespace, isLoadingNamespace } = useLanguage();
  ...
  if (!isAdmin) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="text-gray-600 mb-6">{t('access_denied_desc')}</p>
        <Link href="/">
          <Button className="bg-red-600 hover:bg-red-700">{t('back_home')}</Button>
        </Link>
      </div>
    );
  }
  ...
  const menuItems: Array<{ type: "link"; id: string; item: AdminMenuLink } | { type: "group"; group: AdminMenuGroup }> = adminNamespaceLoaded ? [
    {
      ...
      group: {
        ...
        children: [
          { path: "/admin/dashboard", icon: LayoutDashboard, label: t('dashboard_title') },
          { path: "/admin/statistics", icon: BarChart3, label: t('statistics_title') },
        ],
      },
    },
    {
      ...
      group: {
        ...
        icon: ArrowDownUp,
        children: [
          { path: "/admin/importExport", icon: ArrowDownUp, label: t('import_export_title') },
        ],
      },
    },
    {
      ...
      group: {
        ...
        icon: Package,
        children: [
          { path: "/admin/productsAdmin", icon: Package, label: t('menu_products') },
          { path: "/admin/ordersAdmin", icon: ShoppingCart, label: t('menu_orders') },
          { path: "/admin/customersAdmin", icon: Users, label: t('menu_customers') },
          { path: "/admin/usersAdmin", icon: Shield, label: t('menu_users') },
          { path: "/admin/couponsAdmin", icon: TicketPercent, label: t('menu_coupons') },
        ],
      },
    },
    {
      ...
      group: {
        ...
        icon: Megaphone,
        children: [{ path: "/admin/bannersAdmin", icon: Megaphone, label: t('menu_banners') }],
      },
    },
    {
      type: "group",
      ...
      {isLoadingNamespace('admin-common') && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm">
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium">{t('loading_admin_ui')}</p>
          </div>
        </div>
      )}
      <aside className={`bg-black text-white flex flex-col transition-all duration-300 ease-in-out ${sidebarOpen ? 'w-64' : 'w-20'}`}>
        <div className={`p-4 border-b border-gray-700 flex ${sidebarOpen ? 'flex-row items-center justify-between' : 'flex-col items-center gap-4'}`}>
          <div className={`bg-red-600 text-white px-3 py-2 rounded shrink-0 ${!sidebarOpen && 'w-full text-center'}`}>
            <span className="text-xl">{adminNamespaceLoaded ? t('logo_text') : 'LT'}</span>
          </div>
          {sidebarOpen && adminNamespaceLoaded && (
            <div>
              <div className="text-lg">{t('store_name')}</div>
              <div className="text-xs text-green-100">{t('management')}</div>
            </div>
          )}
          <button
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="text-gray-300 hover:text-white p-2 rounded transition-colors shrink-0"
            title={adminNamespaceLoaded ? (sidebarOpen ? t('sidebar_close') : t('sidebar_open')) : ''}
            aria-label={adminNamespaceLoaded ? (sidebarOpen ? t('sidebar_close') : t('sidebar_open')) : ''}
          >
        ...
        <div className={`border-t border-red-600 bg-red-600 ${sidebarOpen ? 'p-4' : 'p-2'}`}>
          ...
          <Link href="/">
            <Button
              ...
              title={sidebarOpen || !adminNamespaceLoaded ? undefined : t('back_home')}
            >
              <Home className="w-5 h-5 text-white shrink-0" />
              {sidebarOpen && adminNamespaceLoaded && <span>{t('back_home')}</span>}
            </Button>
          </Link>
          <Button
            variant="ghost"
            ...
            onClick={logout}
            title={sidebarOpen || !adminNamespaceLoaded ? undefined : t('logout')}
          >
            <LogOut className="w-5 h-5 text-white shrink-0" />
            {sidebarOpen && adminNamespaceLoaded && <span>{t('logout')}</span>}
          </Button>
        </div>
      </aside>
