...
export function ProductCard({ laptop, onQuickViewToggle }: ProductCardProps) {
  ...
  const convertedLaptop: Laptop = useMemo(() => {
    ...
    const displayCategoryName = categoryObj?.translationKey ? t(categoryObj.translationKey, 'categories') : (categoryName ? t(categoryName, 'categories') : '');
    const displayBrand = laptop.brand === 'Không thương hiệu' ? (locale === 'vi' ? 'Không thương hiệu' : 'No Brand') : t(laptop.brand, 'products');

    return {
      id: laptop._id || laptop.id || '',
      name: (locale !== 'vi' && translation?.name) ? translation.name : (laptop.name ?? ''),
      brand: displayBrand,
      ...
      <RouterOrDiv href={isValidId ? `/product/${convertedLaptop.id}` : '#'} className="group block h-full" isValidId={isValidId}>
        <div className="bg-white border rounded-lg overflow-hidden group-hover:shadow-xl transition-shadow duration-300 flex flex-col h-full relative z-0">
          ...
          <div className="p-2 sm:p-3 flex flex-col flex-1">
            ...
            <div className="flex gap-2 mt-1 text-xs text-gray-500 mb-2">
              {convertedLaptop.categoryName && (
                <span className="bg-gray-100 px-2 py-0.5 rounded text-gray-600 uppercase">{convertedLaptop.categoryName}</span>
              )}
            </div>
            ...
            <div className="flex flex-col justify-end gap-0.5 mb-2 h-10 sm:h-12">
              ...
              <span className="text-green-600 font-bold text-xs sm:text-sm">{formatCurrency(convertedLaptop.price)}</span>
            </div>
            ...
            >
              <span className="flex items-center justify-center gap-1">
                ...
                <span className="hidden sm:inline">{isAddingToCart ? t('adding', 'products') : t('add_to_cart_btn', 'products')}</span>
                <span className="sm:hidden">{isAddingToCart ? t('adding_mobile', 'products') : t('add_to_cart_mobile', 'products')}</span>
              </span>
            </Button>
          </div>
        </div>
