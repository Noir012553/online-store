import { ShoppingCart, X } from "lucide-react";
import { useRouter } from "next/router";
import { useCart } from "../lib/context/CartContext";
import { Badge } from "./ui/badge";
import { formatCurrency } from "../lib/utils";
import { useState, useEffect, useRef } from "react";

export function FloatingCart() {
  const router = useRouter();
  const { totalItems, totalPrice } = useCart();
  const [position, setPosition] = useState({ x: 32, y: 80 });
  const [isDragging, setIsDragging] = useState(false);
  const [dragOffset, setDragOffset] = useState({ x: 0, y: 0 });
  const [isHidden, setIsHidden] = useState(false);
  const [hasMoved, setHasMoved] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);
  const dragStartPos = useRef({ x: 0, y: 0 });

  useEffect(() => {
    const saved = localStorage.getItem("floatingCartState");
    if (saved) {
      const state = JSON.parse(saved);
      setPosition(state.position);
      setIsHidden(state.isHidden);
    }
  }, []);

  const saveState = (newPosition: typeof position, newIsHidden: boolean) => {
    localStorage.setItem(
      "floatingCartState",
      JSON.stringify({ position: newPosition, isHidden: newIsHidden })
    );
  };

  const handleMouseDown = (e: React.MouseEvent) => {
    if ((e.target as HTMLElement).closest("button[data-close-btn]")) {
      return;
    }
    setIsDragging(true);
    setHasMoved(false);
    dragStartPos.current = { x: e.clientX, y: e.clientY };
    if (containerRef.current) {
      const rect = containerRef.current.getBoundingClientRect();
      setDragOffset({
        x: e.clientX - rect.left,
        y: e.clientY - rect.top,
      });
    }
  };

  useEffect(() => {
    if (!isDragging) return;

    const handleMouseMove = (e: MouseEvent) => {
      const distance = Math.sqrt(
        Math.pow(e.clientX - dragStartPos.current.x, 2) +
        Math.pow(e.clientY - dragStartPos.current.y, 2)
      );
      if (distance > 5) {
        setHasMoved(true);
      }

      if (containerRef.current) {
        const newX = e.clientX - dragOffset.x;
        const newY = e.clientY - dragOffset.y;
        setPosition({ x: newX, y: newY });
      }
    };

    const handleMouseUp = () => {
      setIsDragging(false);
      setPosition((prev) => {
        saveState(prev, isHidden);
        return prev;
      });
    };

    document.addEventListener("mousemove", handleMouseMove);
    document.addEventListener("mouseup", handleMouseUp);

    return () => {
      document.removeEventListener("mousemove", handleMouseMove);
      document.removeEventListener("mouseup", handleMouseUp);
    };
  }, [isDragging, dragOffset, isHidden]);

  const handleClose = () => {
    setIsHidden(true);
    saveState(position, true);
  };

  const handleReopen = () => {
    setIsHidden(false);
    saveState(position, false);
  };

  const handleCartClick = () => {
    if (!hasMoved) {
      router.push("/cart");
    }
  };

  if (isHidden) {
    return (
      <div
        style={{
          position: "fixed",
          right: "32px",
          bottom: "150px",
          zIndex: 40,
        }}
      >
        <button
          onClick={handleReopen}
          className="bg-red-600 hover:bg-red-700 text-white w-12 h-12 rounded-full shadow-lg transition-all duration-300 hover:scale-105 flex items-center justify-center"
          title="Mở giỏ hàng"
        >
          <ShoppingCart className="w-5 h-5" />
        </button>
      </div>
    );
  }

  return (
    <div
      ref={containerRef}
      style={{
        position: "fixed",
        left: `${position.x}px`,
        top: `${position.y}px`,
        zIndex: 40,
        cursor: isDragging ? "grabbing" : "grab",
      }}
      className="transition-shadow duration-300 select-none"
      onMouseDown={handleMouseDown}
    >
      <div className="relative inline-block">
        <button
          onClick={handleCartClick}
          className="bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-full shadow-lg transition-all duration-300 hover:scale-105 flex items-center gap-3 group"
          title="Xem giỏ hàng"
        >
          <div className="relative">
            <ShoppingCart className="w-5 h-5" />
            {totalItems > 0 && (
              <Badge className="absolute -top-2 -right-2 bg-white text-red-600 min-w-5 h-5 flex items-center justify-center">
                {totalItems}
              </Badge>
            )}
          </div>
          <div className="hidden sm:block">
            <p className="text-xs opacity-90">Giỏ hàng</p>
            <p className="font-semibold">{formatCurrency(totalPrice)}</p>
          </div>
        </button>
        <button
          data-close-btn
          onClick={handleClose}
          className="absolute -top-2 -right-2 bg-white text-red-600 rounded-full w-6 h-6 flex items-center justify-center shadow-md hover:bg-red-50 transition-colors"
          title="Ẩn giỏ hàng"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
    </div>
  );
}
