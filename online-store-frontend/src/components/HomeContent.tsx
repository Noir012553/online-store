...
import { onBannerCreated, onBannerUpdated, onBannerDeleted, onBannerRestored } from "../lib/socket";
...
export default function Home() {
  ...
  const buildHeroSlides = (): HeroSlide[] => {
    ...
    const gamingCategory = safeCats.find((c: any) => c.name === 'Laptop Gaming');
    const officeCategory = safeCats.find((c: any) => c.name === 'Laptop Văn phòng');

    return [
      {
        sortOrder: 0,
        title: t('gaming_powerhouse_title'),
        subtitle: t('gaming_powerhouse_subtitle'),
        description: t('gaming_powerhouse_desc'),
        image: "https://images.unsplash.com/photo-1593640408182-31c70c8268f5?w=1200",
        cta: t('explore_gaming_laptops'),
        link: gamingCategory ? `/products/${categoryToSlug(gamingCategory.name)}` : "/products",
      },
      {
        sortOrder: 1,
        title: t('professional_productivity_title'),
        subtitle: t('professional_productivity_subtitle'),
        description: t('professional_productivity_desc'),
        image: "https://images.unsplash.com/photo-1520607162513-77705c0f0d4a?w=1200",
        cta: t('browse_office_laptops'),
        link: officeCategory ? `/products/${categoryToSlug(officeCategory.name)}` : "/products",
      },
      {
        sortOrder: 2,
        title: t('innovation_quality_title'),
        subtitle: t('innovation_quality_subtitle'),
        description: t('innovation_quality_desc'),
  ...
  const adminHeroSlides: HeroSlide[] = homepageHeroBanners.map((banner, index) => {
    ...
    return {
      ...
      title: (typeof banner.title === 'object' ? banner.title[currentLang] : banner.title) || (typeof banner.title === 'object' ? banner.title['vi'] : '') || '',
      subtitle: (typeof banner.subtitle === 'object' ? banner.subtitle[currentLang] : banner.subtitle) || (typeof banner.subtitle === 'object' ? banner.subtitle['vi'] : '') || '',
      description: (typeof banner.description === 'object' ? banner.description[currentLang] : banner.description) || (typeof banner.description === 'object' ? banner.description['vi'] : '') || '',
      image: banner.image,
      cta: (typeof banner.ctaText === 'object' ? banner.ctaText[currentLang] : banner.ctaText) || (typeof banner.ctaText === 'object' ? banner.ctaText['vi'] : '') || '',
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const productsResponse = await productAPI.getFeaturedProducts(1, undefined, undefined, undefined, 200);
        ...
        // Find category IDs for office and gaming laptops by name
        const officeCategory = cats.find((c: any) => c.name === 'Laptop Văn phòng');
        const gamingCategory = cats.find((c: any) => c.name === 'Laptop Gaming');
        ...
        const officeProducts = allProductsList.filter((p: BackendProduct) => {
          ...
          const isOffice = officeCategory && catId === officeCategory._id;
          return isOffice && (p.countInStock || 0) > 0;
        });

        const gamingProducts = allProductsList.filter((p: BackendProduct) => {
          const catId = getCategoryId(p.category);
          const isGaming = gamingCategory && catId === gamingCategory._id;
        ...
        const otherProducts = allProductsList.filter((p: BackendProduct) => {
          const catId = getCategoryId(p.category);
          const isLaptop =
            (officeCategory && catId === officeCategory._id) ||
            (gamingCategory && catId === gamingCategory._id);
  ...
  useEffect(() => {
    ...
    const fetchHomepageHeroBanners = async () => {
      try {
        const response = await bannerAPI.getBanners('homepage_hero', true, 1, 10);
        if (!isMounted) return;
            ...
            <section className="bg-white container mx-auto section-container-px py-8 sm:py-12">
              <div className="bg-cyan-100 rounded-lg sm:rounded-2xl p-4 sm:p-8 border-2 border-red-500">
                <div className="text-center mb-6 sm:mb-8">
                  ...
                  <p className="text-lg sm:text-xl mb-3 sm:mb-4 text-black font-bold">{t('limited_time_offer')}</p>
