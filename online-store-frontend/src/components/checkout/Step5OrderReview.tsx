...
export function Step5OrderReview() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const itemsPriceOnly = totalPrice - shippingFee;
      ...
      const orderData = {
        orderItems: items.map((item) => {
          ...
          if (!productId) {
            throw new Error(`Invalid product in cart: ${item.laptop.name || t('payment_method_unknown')}`);
          }
          return {
            product: productId,
      ...
      if (!orderId) {
        throw new Error(t('error_order_id'));
      }
  ...
  const getErrorMessage = (error: any): string => {
    if (error instanceof Error) {
      if (error.message.includes('Email already in use')) {
        return t('email_in_use', 'errors');
      } else if (error.message.includes('Insufficient stock')) {
        return t('insufficient_stock', 'errors');
      } else if (error.message.includes('No order items')) {
        return t('cart_empty', 'errors');
      }
      return t('create_order', 'errors');
    }
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2">
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-6">{t('order_confirm')}</h2>

          <form onSubmit={handleSubmit} className="space-y-8">
            {/* Products */}
            <div>
              <h3 className="font-bold text-lg text-gray-900 mb-4">{t('product_list')}</h3>
      ...
      <div>
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 sticky top-24">
          <h3 className="text-xl font-bold text-gray-900 mb-4">{t('order_summary')}</h3>
