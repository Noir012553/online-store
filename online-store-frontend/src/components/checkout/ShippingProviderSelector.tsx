...
export function ShippingProviderSelector({
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.options && response.options.length > 0) {
          ...
          if (providerServices.length > 0) {
            ...
            if (selectedService?.provider !== activeProvider && providerServices[0]) {
    ...
    loadServices();
  }, [activeProvider, to_districtId, to_wardCode, cartItems, t, locale, currencyCode]);

  const handleProviderChange = (providerCode: string) => {
    if (providerCode === activeProvider) return;
