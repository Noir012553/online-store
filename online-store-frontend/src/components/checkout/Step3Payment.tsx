...
import { generateIdempotencyKey, storeIdempotencyKey, clearIdempotencyKey } from '../../lib/idempotencyKey';
...
export function Step3Payment() {
  ...
  const { formData, goBack, setFormData, couponCode, checkoutSummary } = useCheckout();
  const { items } = useCart();
  ...
  const createOrder = async (idempotencyKey: string) => {
    try {
      ...
      const orderResponse = await apiCall<{ success: boolean; data?: any }>(`/orders?${orderParams.toString()}`, {
        method: 'POST',
  ...
  const createShipment = async (orderId: string) => {
    try {
      ...
      const shipmentResponse = await apiCall<{ success: boolean; order: unknown }>(`/shipments?${shipmentParams.toString()}`, {
        method: 'POST',
  ...
  const handleCOD = async () => {
    if (isLoading || !checkoutSummary) return;
  ...
  const handleVNPAY = async () => {
    ...
    try {
      ...
      const paymentResponse = await apiCall<{ success: boolean; data?: any }>(`/payments/initiate?${paymentParams.toString()}`, {
        method: 'POST',
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-8">
        <div className="flex justify-between gap-4">
          ...
          <Button
            ...
            disabled={isLoading || !selectedMethod || !checkoutSummary}
