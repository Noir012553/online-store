...
export function Step2Payment() {
  ...
  const finalTotal = items.reduce<number | null>((sum, item) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orderResponse = await fetch(`/api/orders?lang=${locale}`, {
        method: 'POST',
        headers: {
      ...
      if (!orderResponse.ok) {
        ...
        throw new Error(error.message || t('error_create_order', 'checkout'));
      }
      ...
      const errorMessage = error instanceof Error ? error.message : t('error_order_processing', 'checkout');
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-gray-700">
                <strong>{t('payment_method_select', 'checkout')}:</strong> {t('payment_cod', 'checkout')}
              </p>
            </div>
            ...
            <div className="flex justify-between">
              ...
              >
                {isSubmitting ? (
                  ...
                  t('confirm_order_button', 'checkout')
                )}
              </Button>
            </div>
          </form>
        ...
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 sticky top-24">
          ...
          <div className="space-y-3 mb-6 pb-6 border-b border-gray-200">
            <div className="flex justify-between items-center text-gray-700">
              ...
              <span className="font-medium">{finalTotal !== null && formatConvertedPrice(finalTotal)}</span>
            </div>
          </div>

          <div className="flex justify-between items-center mb-4">
            <span className="font-bold text-gray-900">{t('total', 'checkout')}</span>
            <span className="text-2xl font-bold text-red-600">{finalTotal !== null && formatConvertedPrice(finalTotal)}</span>
          </div>
