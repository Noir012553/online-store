  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!orderResponse.ok) {
        ...
        throw new Error(error.message || t('error_create_order'));
      }
      ...
      const errorMessage = error instanceof Error ? error.message : t('error_order_processing');
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-gray-700">
                <strong>{t('payment_method_select')}:</strong> {t('payment_cod')}
              </p>
            </div>
