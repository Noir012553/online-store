...
export function CategoryProductsList({ categoryId, categoryName }: CategoryProductsListProps) {
  ...
  const getFormattedPrice = (amount: number) => (
    allProducts.find((product) => product.price === amount)?.formattedPrice
  );
    ...
    <div className="flex flex-col lg:flex-row gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm text-gray-900 font-semibold">
                        <span className="text-red-600">{getFormattedPrice(minPrice || sliderMin)}</span>
                        <span className="text-red-600">{getFormattedPrice(maxPrice || sliderMax)}</span>
                      </div>
                    </div>
                  </div>
                </AccordionContent>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <select value={sortBy} onChange={(event) => setSortBy(event.target.value)} className="border-input flex h-9 w-40 rounded-md border bg-input-background px-3 py-2 text-sm outline-none disabled:cursor-not-allowed disabled:opacity-50">
