...
export function CategoryProductsList({ categoryId, categoryName }: CategoryProductsListProps) {
  ...
  }, [categoryId, page, filters, hasRestoredFilters]);
      ...
      <aside className="w-full lg:w-64 shrink-0">
        <div className="bg-white border rounded-lg overflow-hidden">
          ...
          <div className="px-4 py-3 border-b bg-white flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900">{t('filter_title')}</h3>
            {hasActiveFilters && (
              <Button
                variant="ghost"
                size="sm"
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-gray-600">
            {isLoading ? t('filter_loading_wait') : `${sortedProducts.length} ${t('filter_items_count')}`}
          </p>
          <Select value={sortBy} onValueChange={setSortBy}>
            <SelectTrigger className="w-40">
              <SelectValue placeholder={t('filter_sort_by')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="featured">{t('badge_featured')}</SelectItem>
              <SelectItem value="name">{t('filter_popular')}</SelectItem>
              <SelectItem value="price-asc">{t('sort_price_low_to_high')}</SelectItem>
              <SelectItem value="price-desc">{t('sort_price_high_to_low')}</SelectItem>
              <SelectItem value="rating">{t('filter_stat_rating')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {(filters.discountRange[0] > 0 || filters.discountRange[1] > 0) && (
              ...
              >
                {t('filter_discount_range')}: {filters.discountRange[0]}% - {filters.discountRange[1]}%
                <X className="w-3 h-3" />
              </Badge>
            )}
            {(filters.ratingRange[0] > 0 || filters.ratingRange[1] < 5) && (
              ...
              >
                {t('filter_rating_range')}: {filters.ratingRange[0].toFixed(1)} - {filters.ratingRange[1].toFixed(1)}
                <X className="w-3 h-3" />
              </Badge>
            )}
            {filters.featuredOnly && (
