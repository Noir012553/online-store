...
export function BannerSlot({ slot, variant = 'strip', className = '', limit = 10 }: BannerSlotProps) {
  ...
  useEffect(() => {
    ...
    const loadBanners = async () => {
      setIsLoading(true);
      try {
        const response = await bannerAPI.getBanners(slot, true, 1, limit);
        if (!isMounted) return;
  ...
  const localizedTitle = getLocalizedText(currentBanner.title);
  const localizedSubtitle = getLocalizedText(currentBanner.subtitle);
  const localizedDescription = getLocalizedText(currentBanner.description);
  const localizedCtaText = getLocalizedText(currentBanner.ctaText);
