...
export default function Profile() {
  ...
  const handleAvatarChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`/api/users/avatar?lang=${locale}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <>
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4 sm:gap-6">
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
                  <div className="relative group">
                    {avatarPreview ? (
                      ...
                      >
                        <img
                          src={avatarPreview}
                          alt={user.name || t('user_unnamed', 'profile')}
                      ...
                      <div className={`w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center shadow-lg border-4 border-white ${isAdmin ? 'bg-white' : 'bg-red-600'}`}>
                        <span className={`text-3xl sm:text-4xl font-bold ${isAdmin ? 'text-red-600' : 'text-white'}`}>{(user.name || t('user_unnamed', 'profile'))[0]?.toUpperCase()}</span>
                      </div>
                    )}

                    <label htmlFor="avatar-upload" className="absolute bottom-0 right-0 rounded-full bg-black/50 p-2 opacity-0 group-hover:opacity-100 cursor-pointer transition-opacity">
                  ...
                  <div className="text-white text-center sm:text-left flex-grow">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-1 sm:mb-2">{user?.name || t('user_unnamed', 'profile')}</h1>
                    ...
                    {isAdmin ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="p-4 bg-white border border-gray-200 rounded-lg">
                          ...
                          <p className="text-gray-900 font-semibold text-sm sm:text-base">{t('profile_joined_date', 'profile')}</p>
                        </div>
                      </div>
                    )}
      ...
      {isImageViewerOpen && avatarPreview && (
        <ImageViewer
          src={avatarPreview}
          alt={user.name || t('user_unnamed', 'profile')}
