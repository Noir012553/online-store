...
export default function Cart() {
  ...
  const getSummaryItem = (item: (typeof items)[number]) =>
    summary?.orderItems.find((summaryItem) =>
      String(summaryItem.product) === String(item.laptop.id || item.laptop._id)
    );
                ...
                <div key={item.laptop.id} className="p-3 md:p-4 hover:bg-white transition-colors animate-in slide-in-from-right duration-300">
                  <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-center">
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center">
                        ...
                        <div className="md:hidden mt-1">
                          <p className="text-red-600 text-xs">
                            {getSummaryItem(item)?.formattedPrice}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="hidden md:block md:col-span-2 text-left text-xs">
                      <span className="text-gray-900">{getSummaryItem(item)?.formattedPrice}</span>
                    </div>
                    ...
                    <div className="md:col-span-1 flex items-center justify-start">
                      <div className="text-left">
                        ...
                        <span className="text-red-600 text-xs">
                          {getSummaryItem(item)?.formattedLineTotal}
                        </span>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-lg border p-6 sticky top-24">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between">
                ...
                <span>{summary?.formattedItemsPrice}</span>
              </div>
              <div className="border-t pt-3 flex justify-between">
                <span>{t('total', 'cart')}</span>
                <span className="text-red-600">{summary?.formattedTotalPrice}</span>
              </div>
            </div>
