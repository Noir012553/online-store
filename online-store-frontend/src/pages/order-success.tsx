...
export default function OrderSuccess() {
  ...
  const [spendingSummary, setSpendingSummary] = useState<Array<{ currencyCode: string; formattedTotalPrice: string }>>([]);
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      setError(null);
      try {
        const data = await orderAPI.getMyOrders(locale, getIntlLocale(locale), currencyCode);
        const orderList = Array.isArray(data) ? data : data?.data || data?.orders || [];
        setSpendingSummary(data?.summary?.totalSpentByCurrency || []);
    ...
    fetchOrders();
  }, [isInitialized, user, router, locale, currencyCode]);
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg border shadow-sm p-4">
            ...
            <div className="text-2xl font-bold text-gray-900">
              {spendingSummary.map(({ currencyCode, formattedTotalPrice }) => (
                <p key={currencyCode}>{formattedTotalPrice}</p>
              ))}
            </div>
