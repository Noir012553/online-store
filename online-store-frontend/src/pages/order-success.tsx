  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : t('error_load_orders');
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white rounded-lg border shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-gray-600 text-sm font-medium">{t('stats_total_orders')}</span>
              <ShoppingBag className="w-5 h-5 text-red-600" />
            </div>
            <p className="text-2xl font-bold text-gray-900">{totalOrders}</p>
            <p className="text-xs text-gray-500 mt-1">{t('stats_orders_label')}</p>
          </div>

          {/* Total Spent */}
          <div className="bg-white rounded-lg border shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-gray-600 text-sm font-medium">{t('stats_total_spent')}</span>
              <Wallet className="w-5 h-5 text-blue-600" />
            </div>
            <p className="text-2xl font-bold text-gray-900">{formatCurrency(totalSpent)}</p>
            <p className="text-xs text-gray-500 mt-1">{t('stats_all_orders_label')}</p>
          </div>

          {/* Delivered Orders */}
          <div className="bg-white rounded-lg border shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-gray-600 text-sm font-medium">{t('stats_delivered')}</span>
          ...
          <div className="bg-white rounded-lg border shadow-sm p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-gray-600 text-sm font-medium">{t('stats_pending')}</span>
        ...
        {newOrder && (
          <div className="bg-linear-to-r from-green-50 to-blue-50 rounded-lg border border-green-200 p-6 mb-8">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('new_order_title')}</h2>
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('new_order_order_id')}</p>
                <p className="text-lg font-bold text-red-600 font-mono mb-4">{newOrder._id}</p>

                <p className="text-sm text-gray-600 mb-1">{t('new_order_order_date')}</p>
              ...
              <div>
                <p className="text-sm text-gray-600 mb-1">{t('new_order_status')}</p>
                <div className="mb-4">
                ...
                <p className="text-sm text-gray-600 mb-1">{t('new_order_total')}</p>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <p className="text-sm font-medium text-gray-900 mb-3">{t('new_order_items_title')} ({newOrder.orderItems.length})</p>
        ...
        <div className="bg-white rounded-lg border shadow-sm p-6 mb-8">
          <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('recent_orders_title')}</h2>

          {recentOrders.length === 0 ? (
            <p className="text-center text-gray-500 py-8">{t('recent_orders_empty')}</p>
          ) : (
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          ...
          <ul className="space-y-2 text-blue-800 text-sm">
            <li className="flex items-start gap-2">
              ...
              <span>{t('next_steps_email_confirmation')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="font-bold mt-0.5">✓</span>
              <span>{t('next_steps_complete_payment')}</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="font-bold mt-0.5">✓</span>
              <span>{t('next_steps_track_orders')}</span>
            </li>
          </ul>
        </div>
      </div>
