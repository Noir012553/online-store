...
export default function OrderConfirmation() {
  ...
  useEffect(() => {
    ...
    const initializeOrder = async () => {
      try {
        ...
        if (!orderData && user) {
          try {
            const response = await orderAPI.getOrder(orderId as string, locale);
        ...
        toast.error(t('error_load_data', 'order-confirmation'));
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8 animate-in fade-in duration-300">
      <Breadcrumbs
        links={[
          { label: t('home', 'order-confirmation'), href: "/" },
          { label: t('breadcrumb_order_confirmation', 'order-confirmation') },
        ]}
      />
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold mb-2 text-green-600">
            {t('order_confirmation_title', 'order-confirmation')}
          </h1>
          <p className="text-gray-600">
            {t('order_confirmation_thank_you', 'order-confirmation')}
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg border shadow-sm p-6 mb-6">
          ...
          {/* Order Items */}
          <div className="mb-6">
            <h3 className="font-semibold mb-4">{t('order_items_details', 'order-confirmation')}</h3>
              ...
              {order.orderItems.map((item, index) => (
                <div key={index} className="flex justify-between items-center pb-3 border-b last:border-b-0">
                  ...
                  <p className="font-semibold text-red-600">
                    {formatOrderPrice(item.price * item.qty, order.currencyCode)}
                  </p>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-lg p-4 space-y-2">
            <div className="flex justify-between text-lg font-bold text-red-600">
              ...
              <span>{formatOrderPrice(order.totalPrice, order.currencyCode)}</span>
            </div>
          </div>
        </div>
