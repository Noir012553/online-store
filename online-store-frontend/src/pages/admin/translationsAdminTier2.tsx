...
const SUPPORTED_LANGUAGES = [
  { code: 'en', nameKey: 'admin_tier2_language_name_english' },
];

const TranslationsAdminTier2 = () => {
  ...
  const fetchCacheStats = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/translations/admin/cache-stats`, {
        method: 'POST',
        headers: {
  ...
  const fetchCacheRecords = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/translations/admin/cache-records?lang=${selectedLang}&skip=${skip}&limit=${pageSize}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const handleDeleteRecord = async (id: string) => {
    if (!confirm(t('admin_tier2_confirm_delete_cache_record', 'admin-translation'))) return;
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/translations/admin/cache/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handleClearOldCache = async () => {
    if (!confirm(t('admin_tier2_confirm_clear_old_cache', 'admin-translation'))) return;
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/translations/admin/clear-cache`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const getLanguageName = (code: string) => {
    ...
    return lang ? t(lang.nameKey, 'admin-translation') : code;
  };
            ...
            >
              {SUPPORTED_LANGUAGES.map((lang) => (
                <option key={lang.code} value={lang.code}>
                  {t(lang.nameKey, 'admin-translation')}
                </option>
              ))}
            </select>
          </div>
                        ...
                        <tr className={`tier2-expanded-row show`}>
                          <td colSpan={6}>
                            <div className="tier2-expanded-content">
                              ...
                              <div className="tier2-expanded-field">
                                <label>{t('admin_tier2_expanded_translated_text', 'admin-translation').replace('{lang}', record.targetLang.toUpperCase())}</label>
...
export default TranslationsAdminTier2;
