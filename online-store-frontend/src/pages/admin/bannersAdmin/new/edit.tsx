...
const getIncompleteLanguages = (banner: BannerFormState, hasCTA: boolean): Locale[] => {
...
const validateMultiLanguage = (banner: BannerFormState, hasCTA: boolean): string | null => {
  ...
  const hasAnyTitle = SUPPORTED_LOCALES.some(lang => banner.title[lang]?.trim());
  if (!hasAnyTitle) {
    return 'banner_error_lang_at_least_one_title';
  }

  if (isHeroSlot && hasCTA) {
    const hasCompleteInAnyLang = SUPPORTED_LOCALES.some(lang => {
      const status = getLanguageCompletionStatus(banner, true, lang);
      return status === 'complete';
    });
    if (!hasCompleteInAnyLang) {
      return 'banner_error_lang_at_least_one_complete_lang';
    }
  }
...
const createEmptyForm = (): BannerFormState => {
  ...
  return {
    title: createEmptyMultiLangField(),
    subtitle: createEmptyMultiLangField(),
    description: createEmptyMultiLangField(),
    ctaText: createEmptyMultiLangField(),
    targetUrl: '',
    slot: DEFAULT_SLOT,
    sortOrder: '0',
    isActive: true,
...
function BannerCreatePageContent() {
  ...
  const handleSubmit = async () => {
    ...
    if (!imageFile && !imagePreview) {
      toast.error(t('banner_error_no_image', 'admin-banners'));
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-12 p-6">
        <div className="text-gray-500">{t('banner_loading', 'admin-banners')}</div>
      </div>
    );
  }
      ...
      <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold text-gray-900">
              {t('banner_add_title', 'admin-banners')}
            </h1>
          </div>
        </div>
            ...
            <div className="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
              ...
              <div className="text-xs text-gray-600 space-y-1 pt-2">
                ...
                <div className="mt-2 pt-2 border-t border-gray-200 text-gray-500">
                  {t('banner_form_lang_all_supported_note', 'admin-banners') || 'Tất cả 9 ngôn ngữ được hỗ trợ đều có sẵn'}
                </div>
              </div>
        ...
        {banner && getIncompleteLanguages(banner, selectedSlotGuidance?.hasCTA || false).length > 0 && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
            <div className="text-sm text-amber-800">
              <div className="font-semibold">{t('banner_form_warning_incomplete_languages', 'admin-banners') || 'Cảnh báo: Một số ngôn ngữ chưa hoàn tất'}</div>
              <div className="mt-2 space-y-1 text-xs">
                {getIncompleteLanguages(banner, selectedSlotGuidance?.hasCTA || false).map(lang => (
                  <div key={lang} className="flex items-center gap-2">
                    <span>{AVAILABLE_LOCALES[lang].flag}</span>
                    <span>{t(AVAILABLE_LOCALES[lang].labelKey, 'admin-banners')} ({lang.toUpperCase()})</span>
                  </div>
                ))}
              </div>
              <div className="mt-3 text-xs text-amber-700">{t('banner_form_warning_incomplete_finish_note', 'admin-banners') || 'Bạn có thể hoàn tất những ngôn ngữ này sau từ trang quản lý bản dịch'}</div>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {isSubmitting ? t('banner_dialog_confirm_save', 'admin-banners') : t('banner_form_create', 'admin-banners')}
          </Button>
      </div>
    </div>
  );
...
const BannerCreatePage = () => {
  return <BannerCreatePageContent />;
};

export default withAdminLayout(BannerCreatePage, {
  permission: 'manage:banners',
  featureName: 'Create Banner'
});
