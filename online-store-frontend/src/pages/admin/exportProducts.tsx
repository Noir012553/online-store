...
export default function ExportProducts() {
  ...
  const fetchExportStats = async () => {
    try {
      const stats = await productAPI.getExportStats();
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      const data = await productAPI.exportProducts(selectedFormat, selectedCategory);
      ...
      toast.success(t('exporting', 'export'));
    } catch (error) {
    ...
    <AdminLayout>
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">{t('export_products', 'export')}</h1>
            <p className="text-gray-600 mt-1">{t('export_products_desc', 'export')}</p>
          </div>
        </div>
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  >
                    ...
                    {exportStats?.categories?.map((cat: any) => (
