...
export default function ImportProducts() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [guideData, formatsData] = await Promise.all([
          apiCall('/products/admin/import-guide'),
          apiCall('/products/admin/import-formats'),
        ]);
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const res = await fetch(`/api/products/admin/import-template?format=${format}`, {
        headers,
      });
      ...
      toast.success(t('save_success', 'admin-common'));
    } catch (err) {
  ...
  const handleDirectFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/products/admin/import-file', {
        method: 'POST',
        headers: {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/products/admin/import', {
        method: 'POST',
        headers,
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold mb-8">{t('products_title', 'import')}</h1>
          ...
          <div className="space-y-2 text-sm">
            ...
            <div>
              ...
              <ul className="list-disc ml-5">
                <li>insert: {t('mode_insert_desc')}</li>
                <li>update: {t('mode_update_desc')}</li>
                <li>upsert: {t('mode_upsert_desc')}</li>
              </ul>
            </div>
            <p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="border rounded-lg p-4 bg-blue-50">
            ...
            <label htmlFor="file-upload" className="mb-4 p-3 border-2 border-dashed border-blue-300 rounded bg-white hover:bg-blue-50 transition block text-center cursor-pointer">
              ...
              <p className="text-sm text-blue-600 font-medium">
                {isLoading ? t('exporting', 'export') : `📁 ${t('choose_file')}`}
              </p>
            ...
            <div className="bg-white p-3 rounded text-xs text-gray-600 space-y-1">
              ...
              <ul className="list-disc ml-4">
                <li>{t('format_label')} JSON (.json)</li>
                <li>{t('format_label')} CSV (.csv)</li>
        ...
        <div className="lg:col-span-2">
          ...
          <p className="text-xs text-gray-500 mt-2">
            {t('import_data_hint', 'admin').replace('{{format}}', format.toUpperCase())}
          </p>
          ...
          <div className="flex gap-4 mt-6">
            ...
            >
              {isLoading ? t('processing') : (dryRun ? `👁️ ${t('preview_label').replace(':', '')}` : `▶️ ${t('start', 'admin-import')}`)}
            </Button>
