...
const I18nMonitoring = () => {
  ...
  const fetchHealthData = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/health/i18n`, {
        headers: {
      ...
      setMessage({
        type: 'error',
        text: error.message || 'Failed to fetch health data',
      });
      setLoading(false);
    }
  };
    ...
    return (
      <AdminLayout>
        <div className="monitoring-loading">
          ...
          <p>{t('admin_monitoring_loading', 'Loading monitoring dashboard...')}</p>
        </div>
      </AdminLayout>
    );
  }
    ...
    return (
      <AdminLayout>
        <div className="monitoring-error">
          <AlertCircle size={48} />
          <p>{t('admin_monitoring_error', 'Failed to load monitoring data')}</p>
          <button onClick={fetchHealthData} className="monitoring-retry-btn">
            {t('admin_monitoring_retry', 'Retry')}
          </button>
        </div>
      </AdminLayout>
    );
    ...
    <AdminLayout>
      <div className="monitoring-container">
        <div className="monitoring-header">
          <div>
            <h1 className="monitoring-title">
              {t('admin_monitoring_title', 'i18n System Monitoring')}
            </h1>
            <p className="monitoring-subtitle">
              {t('admin_monitoring_subtitle', 'Real-time performance and health metrics')}
            </p>
          </div>
          <div className="monitoring-controls">
            <button
              ...
              disabled={loading}
              title={t('admin_monitoring_refresh', 'Refresh')}
            >
              <RefreshCw size={18} className={loading ? 'monitoring-spin' : ''} />
              {t('admin_monitoring_refresh', 'Refresh')}
            </button>
            ...
            <label className="monitoring-auto-refresh">
              ...
              />
              <span>{t('admin_monitoring_auto_refresh', 'Auto-refresh')}</span>
            </label>
        ...
        <div className="monitoring-status-card" style={{ borderLeftColor: statusColor[health.status] }}>
          <div className="monitoring-status-header">
            <div className="monitoring-status-info">
              ...
              <div>
                <h2>{t('admin_monitoring_system_status', 'System Status')}</h2>
        ...
        <div className="monitoring-metrics-grid">
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_error_rate', 'Error Rate')}</h3>
            ...
            <div className={`monitoring-metric-status ${health.metrics.errorRate > 5 ? 'critical' : health.metrics.errorRate > 1 ? 'warning' : 'healthy'}`}>
              {health.metrics.errorRate > 5
                ? t('admin_monitoring_critical', 'Critical')
                : health.metrics.errorRate > 1
                ? t('admin_monitoring_warning', 'Warning')
                : t('admin_monitoring_normal', 'Normal')}
            </div>
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_cache_hit_rate', 'Cache Hit Rate')}</h3>
              {metricsHistory.length >= 2 && getTrendIcon(
            ...
            <div className={`monitoring-metric-status ${health.metrics.cacheHitRate < 80 ? 'critical' : health.metrics.cacheHitRate < 90 ? 'warning' : 'healthy'}`}>
              {health.metrics.cacheHitRate < 80
                ...
                ? t('admin_monitoring_warning', 'Warning')
                : t('admin_monitoring_healthy', 'Healthy')}
            </div>
            <div className="monitoring-metric-bar">
              <div
                className="monitoring-metric-fill"
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_api_latency', 'API Latency')}</h3>
              {metricsHistory.length >= 2 && getTrendIcon(
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_queue_length', 'Queue Length')}</h3>
            </div>
            <div className="monitoring-metric-value">
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_db_latency', 'Database Latency')}</h3>
              {metricsHistory.length >= 2 && getTrendIcon(
          ...
          <div className="monitoring-metric-card">
            <div className="monitoring-metric-header">
              <h3>{t('admin_monitoring_memory_usage', 'Memory Usage')}</h3>
            </div>
            <div className="monitoring-metric-value">
        ...
        <div className="monitoring-stats-section">
          <h2>{t('admin_monitoring_request_stats', 'Request Statistics')}</h2>
          <div className="monitoring-stats-grid">
            <div className="monitoring-stat-item">
              <div className="monitoring-stat-label">{t('admin_monitoring_total_requests', 'Total Requests')}</div>
              <div className="monitoring-stat-value">{health.metrics.totalRequests.toLocaleString()}</div>
              <small>{t('admin_monitoring_success', 'Success')}: {health.metrics.successCount} | {t('admin_monitoring_failure', 'Failure')}: {health.metrics.failureCount}</small>
            </div>
            <div className="monitoring-stat-item">
              <div className="monitoring-stat-label">{t('admin_monitoring_rate_limited', 'Rate Limited')}</div>
              <div className="monitoring-stat-value">{health.metrics.rateLimitedRequests}</div>
              <small>{((health.metrics.rateLimitedRequests / health.metrics.totalRequests) * 100).toFixed(2)}% of total</small>
            </div>
            <div className="monitoring-stat-item">
              <div className="monitoring-stat-label">{t('admin_monitoring_retried', 'Retried')}</div>
              <div className="monitoring-stat-value">{health.metrics.retriedRequests}</div>
              <small>{((health.metrics.retriedRequests / health.metrics.totalRequests) * 100).toFixed(2)}% of total</small>
            </div>
          </div>
        </div>
        ...
        {Object.keys(health.languageMetrics).length > 0 && (
          <div className="monitoring-language-section">
            <h2>{t('admin_monitoring_language_metrics', 'Language Metrics')}</h2>
                ...
                <div key={lang} className="monitoring-language-card">
                  <div className="monitoring-language-header">
                    ...
                    <span className={`monitoring-language-status ${metrics.errorRate > 5 ? 'critical' : metrics.errorRate > 1 ? 'warning' : 'healthy'}`}>
                      {metrics.errorRate.toFixed(2)}% error
                    </span>
                  </div>
                  <div className="monitoring-language-metrics">
                    <div>
                      <span className="monitoring-language-label">{t('admin_monitoring_translations', 'Translations')}</span>
                      <span className="monitoring-language-value">{metrics.translationCount.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="monitoring-language-label">{t('admin_monitoring_cache_size', 'Cache Size')}</span>
                      <span className="monitoring-language-value">{(metrics.cacheSize / 1024 / 1024).toFixed(2)}MB</span>
                    </div>
                    <div>
                      <span className="monitoring-language-label">{t('admin_monitoring_last_updated', 'Last Updated')}</span>
        ...
        {health.alerts.length > 0 && (
          <div className="monitoring-alerts-section">
            <h2>{t('admin_monitoring_alerts', 'Alerts')}</h2>
