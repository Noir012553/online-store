...
function CreateUserContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiCall(`/users/admin/create?lang=${locale}`, {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <Label htmlFor="username" className="text-gray-700 font-medium">
              {t('username_label')} <span className="text-red-600">*</span>
            </Label>
            <Input
              id="username"
              type="text"
          ...
          <div>
            <Label htmlFor="confirmPassword" className="text-gray-700 font-medium">
              {t('confirm_password_label')} <span className="text-red-600">*</span>
            </Label>
            <Input
              id="confirmPassword"
              type="password"
          ...
          <div className="flex gap-4 pt-4">
            ...
            >
              {isSubmitting ? t('creating_user_button') : t('create_user_button')}
            </Button>
          </div>
        </form>
      </div>
      ...
      <Dialog open={successDialog} onOpenChange={setSuccessDialog}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>{t('user_created_success')}</DialogTitle>
            <DialogDescription>
              {t('user_creation_success_message')}
            </DialogDescription>
          </DialogHeader>
          ...
          {newUserInfo && (
            <div className="space-y-3 bg-gray-50 p-4 rounded">
              <div>
                <p className="text-sm text-gray-600">{t('email_label')}</p>
                <p className="font-medium text-gray-900">{newUserInfo.email}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">{t('username_label')}</p>
...
export default withAdminLayout(CreateUserContent, {
