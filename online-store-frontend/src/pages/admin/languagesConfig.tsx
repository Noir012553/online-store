...
const LanguagesConfigContent = () => {
  const { t, loadNamespace, locale, refreshLocaleConfig } = useLanguageTranslation();
  ...
  const handleSaveLanguageName = async () => {
    ...
    if (!name || name === editingLanguage.name) {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(`${API_BASE}/api/languages/${editingLanguage._id}?lang=${locale}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ name, isActive: editingLanguage.isActive }),
      });
  ...
  const handleDeleteLanguage = (language: Language) => {
    if (!language.isSystemDefault) setDeletingLanguage(language);
  };
  ...
  const handleConfirmDeleteLanguage = async () => {
    ...
    try {
      const token = getAuthToken();
      const response = await fetch(`${API_BASE}/api/languages/${deletingLanguage._id}?lang=${locale}`, {
        method: 'DELETE',
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <Dialog open={deletingLanguage !== null} onOpenChange={(open) => !open && !isProcessing && setDeletingLanguage(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('admin_delete_language', 'admin-translation')}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-gray-600">
            {deletingLanguage && `${t('admin_delete_language', 'admin-translation')} ${deletingLanguage.name}?`}
          </p>
          <DialogFooter>
            <button
              type="button"
              ...
              className="rounded-lg border border-gray-300 px-4 py-2 font-medium hover:bg-gray-50 disabled:opacity-50"
            >
            ...
            >
              {isProcessing ? t('admin_processing', 'admin-translation') : t('delete', 'admin')}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <Dialog open={editingLanguage !== null} onOpenChange={(open) => !open && !isProcessing && setEditingLanguage(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{t('admin_language_display_name', 'admin-translation')}</DialogTitle>
          </DialogHeader>
          <div>
            <label htmlFor="language-display-name" className="mb-2 block text-sm font-medium text-gray-700">
              {t('admin_language_display_name', 'admin-translation')}
            </label>
            <input
              ...
              disabled={isProcessing}
              autoFocus
              className="w-full rounded-lg border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500 disabled:bg-gray-100"
            />
          </div>
          <DialogFooter>
            <button
            ...
            <button
              type="button"
              onClick={handleSaveLanguageName}
              disabled={isProcessing || !editingLanguageName.trim()}
              className="rounded-lg bg-red-600 px-4 py-2 font-medium text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:bg-gray-400"
            >
              {isProcessing ? t('admin_processing', 'admin-translation') : t('admin_save', 'admin')}
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
