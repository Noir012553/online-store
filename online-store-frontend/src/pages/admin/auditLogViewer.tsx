...
const AuditLogViewer = () => {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error.message || 'Failed to fetch audit logs',
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Timestamp', 'User', 'Action', 'Entity ID', 'Language', 'Old Value', 'New Value', 'Reason'];
    ...
    setMessage({
      type: 'success',
      text: `Exported ${logs.length} logs`,
    });
  };
    ...
    <AdminLayout>
      <div className="audit-log-container">
        <div className="audit-log-header">
          <div>
            <h1 className="audit-log-title">{t('admin_audit_log_title', 'Translation Audit Log')}</h1>
            <p className="audit-log-subtitle">
              {t('admin_audit_log_subtitle', 'View all translation changes and admin overrides')}
            </p>
          </div>
          <div className="audit-log-actions">
            <button
              ...
              title={t('admin_audit_log_toggle_filters', 'Toggle Filters')}
            >
              <Filter size={18} />
              {t('admin_audit_log_filters', 'Filters')}
            </button>
            <button
              onClick={handleRefresh}
              className="audit-log-refresh-btn"
              disabled={loading}
              title={t('admin_audit_log_refresh', 'Refresh')}
            >
            ...
            <button
              ...
              title={t('admin_audit_log_export_csv', 'Export as CSV')}
            >
              <Download size={18} />
              {t('admin_audit_log_export', 'Export')}
            </button>
          </div>
        </div>
          ...
          <div className="audit-log-filters">
            <div className="audit-log-filters-grid">
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_search', 'Search')}</label>
                <input
                  type="text"
                  placeholder={t('admin_audit_log_search_placeholder', 'Search entity ID or username...')}
              ...
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_language', 'Language')}</label>
                <select
                  value={filters.selectedLang}
                  onChange={e => handleFilterChange('selectedLang', e.target.value)}
                  className="audit-log-filter-select"
                >
                  <option value="">{t('admin_audit_log_all_languages', 'All Languages')}</option>
                  {SUPPORTED_LANGUAGES.map(lang => (
                    <option key={lang.code} value={lang.code}>
                      {t(lang.name, lang.code)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_action_type', 'Action Type')}</label>
                <select
                  value={filters.selectedAction}
                  onChange={e => handleFilterChange('selectedAction', e.target.value)}
                  className="audit-log-filter-select"
                >
                  <option value="">{t('admin_audit_log_all_actions', 'All Actions')}</option>
                  {ACTION_TYPES.map(action => (
                    <option key={action.value} value={action.value}>
                      {t(`admin_audit_log_action_${action.value}`, action.label)}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_date_from', 'Date From')}</label>
                <input
                  type="date"
              ...
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_date_to', 'Date To')}</label>
                <input
                  type="date"
              ...
              <div className="audit-log-filter-group">
                <label>{t('admin_audit_log_user_id', 'User ID')}</label>
                <input
                  type="text"
                  placeholder={t('admin_audit_log_user_id_placeholder', 'Filter by user ID...')}
            ...
            <div className="audit-log-filter-actions">
              <button
                ...
                disabled={loading}
              >
                {loading ? t('admin_audit_log_loading', 'Loading...') : t('admin_audit_log_apply_filters', 'Apply Filters')}
              </button>
              <button
                onClick={() => {
                  setFilters({
              ...
              >
                {t('admin_audit_log_reset_filters', 'Reset')}
              </button>
            </div>
          </div>
        )}
        ...
        <div className="audit-log-content">
          {loading && !logs.length ? (
            <div className="audit-log-loading">
              ...
              <p>{t('admin_audit_log_loading', 'Loading audit logs...')}</p>
            </div>
          ) : logs.length === 0 ? (
            <div className="audit-log-empty">
              <p>{t('admin_audit_log_no_records', 'No audit log records found')}</p>
            </div>
          ) : (
            <>
              <div className="audit-log-table-wrapper">
                <table className="audit-log-table">
                  <thead>
                    <tr>
                      <th>{t('admin_audit_log_col_timestamp', 'Timestamp')}</th>
                      <th>{t('admin_audit_log_col_user', 'User')}</th>
                      <th>{t('admin_audit_log_col_action', 'Action')}</th>
                      <th>{t('admin_audit_log_col_entity', 'Entity')}</th>
                      <th>{t('admin_audit_log_col_language', 'Language')}</th>
                      <th>{t('admin_audit_log_col_changes', 'Changes')}</th>
                      <th>{t('admin_audit_log_col_details', 'Details')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {logs.map(log => (
                        ...
                        <tr className="audit-log-table-row">
                          ...
                          <td className="audit-log-action">
                            <span className={`audit-log-action-badge audit-log-action-${log.action}`}>
                              {t(`admin_audit_log_action_${log.action}`, log.action)}
                            </span>
                          </td>
                          ...
                          <td className="audit-log-lang">
                            <span className="audit-log-lang-badge">{log.targetLang.toUpperCase()}</span>
                          </td>
                          ...
                          <td className="audit-log-expand">
                            <button
                              ...
                              title={t('admin_audit_log_view_details', 'View Details')}
                            >
                              ...
                              <div className="audit-log-details">
                                <div className="audit-log-details-grid">
                                  <div className="audit-log-detail-item">
                                    <span className="audit-log-detail-label">{t('admin_audit_log_entity_type', 'Entity Type')}:</span>
                                    <span className="audit-log-detail-value">{log.entityType || '-'}</span>
                                  </div>
                                  <div className="audit-log-detail-item">
                                    <span className="audit-log-detail-label">{t('admin_audit_log_reason', 'Reason')}:</span>
                                    <span className="audit-log-detail-value">{log.reason || t('admin_audit_log_no_reason', 'Not provided')}</span>
                                  </div>
                                  <div className="audit-log-detail-item">
                                    <span className="audit-log-detail-label">{t('admin_audit_log_ip_address', 'IP Address')}:</span>
                                    <span className="audit-log-detail-value">{log.ipAddress || '-'}</span>
                                  </div>
                                  <div className="audit-log-detail-item">
                                    <span className="audit-log-detail-label">{t('admin_audit_log_user_agent', 'User Agent')}:</span>
                                  ...
                                  {log.oldValue && (
                                    <div className="audit-log-detail-item audit-log-full-width">
                                      <span className="audit-log-detail-label">{t('admin_audit_log_old_value', 'Old Value')}:</span>
                                      <div className="audit-log-detail-value-full">{log.oldValue}</div>
                                    </div>
                                  )}
                                  {log.newValue && (
                                    <div className="audit-log-detail-item audit-log-full-width">
                                      <span className="audit-log-detail-label">{t('admin_audit_log_new_value', 'New Value')}:</span>
              ...
              <div className="audit-log-pagination">
                <div className="audit-log-pagination-info">
                  {t('admin_audit_log_showing', 'Showing')} {pagination.pageIndex * pagination.pageSize + 1} {t('admin_audit_log_to', 'to')} {Math.min((pagination.pageIndex + 1) * pagination.pageSize, pagination.totalCount)} {t('admin_audit_log_of', 'of')} {pagination.totalCount}
                </div>
                <div className="audit-log-pagination-buttons">
                  <button
                    onClick={() => setPagination(prev => ({ ...prev, pageIndex: Math.max(0, prev.pageIndex - 1) })) || fetchLogs()}
                    disabled={pagination.pageIndex === 0 || loading}
                    className="audit-log-pagination-btn"
                  >
                    {t('admin_audit_log_previous', 'Previous')}
                  </button>
                  <span className="audit-log-pagination-current">
                    {t('admin_audit_log_page', 'Page')} {pagination.pageIndex + 1} {t('admin_audit_log_of', 'of')} {totalPages}
                  </span>
                  <button
                    onClick={() => setPagination(prev => ({ ...prev, pageIndex: Math.min(totalPages - 1, prev.pageIndex + 1) })) || fetchLogs()}
                    disabled={pagination.pageIndex >= totalPages - 1 || loading}
                    className="audit-log-pagination-btn"
                  >
                    {t('admin_audit_log_next', 'Next')}
                  </button>
                </div>
              </div>
            </>
