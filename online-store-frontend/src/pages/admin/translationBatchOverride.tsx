...
import { Upload, Download, Play, Check, AlertCircle, X, Eye, FileDown, ChevronLeft, ChevronRight, Circle } from 'lucide-react';
...
const TranslationBatchOverride = () => {
  ...
  const confirmProcessBatch = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/translations/admin/batch-manual-override?lang=${locale}`, {
        method: 'POST',
        headers: {
                    ...
                    <tbody>
                      {visibleRows.map((row, visibleIndex) => {
                        ...
                        <tr key={idx} className={`batch-row batch-row-${row.status}`}>
                          ...
                          <td>
                            {row.status === 'success' && (
                              <span className="batch-status-success"><Check className="mr-1 inline h-4 w-4 align-text-bottom" aria-hidden="true" />{t('admin_batch_status_success', 'admin-translation-batch')}</span>
                            )}
                            {row.status === 'error' && (
                              <span className="batch-status-error" title={row.error}><X className="mr-1 inline h-4 w-4 align-text-bottom" aria-hidden="true" />{t('admin_batch_status_error', 'admin-translation-batch')}</span>
                            )}
                            {row.status === 'pending' && (
                              <span className="batch-status-pending"><Circle className="mr-1 inline h-3.5 w-3.5 align-text-bottom" aria-hidden="true" />{t('admin_batch_status_pending', 'admin-translation-batch')}</span>
                            )}
                          </td>
                          <td>
                            <button
