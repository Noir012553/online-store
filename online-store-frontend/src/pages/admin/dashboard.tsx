...
function DashboardContent() {
  ...
  const dashboardLocale = locale as Locale;
  const { currencyCode: targetCurrency } = useCurrencyContext();
  const formatPercentage = (value: number) =>
    new Intl.NumberFormat(getIntlLocale(dashboardLocale), {
      style: 'percent',
      signDisplay: 'always',
  ...
  const fetchDashboardDataCallback = useCallback(async () => {
    try {
      ...
      setStats({
        ...
        formattedTotalRevenue: dashboardData.stats.formattedTotalRevenue || '',
  ...
  useEffect(() => {
    ...
    return () => {
      offEvent('order-deleted', handleOrderDeleted);
  ...
  const getDateRangeDisplay = () => {
    const dateLocale = getIntlLocale(dashboardLocale);
    if (timeFrame === 'day') {
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={revenueData}>
                ...
                <Tooltip
                  ...
                  formatter={(_, __, item: any) => [item?.payload?.formattedRevenue || '', t('revenue_label', 'admin')]}
                />
              ...
              <tbody className="divide-y">
                {filteredRecentOrders.map((order, idx) => (
                  <tr key={order._id || `order-${idx}`} className="text-sm">
                    ...
                    <td className="px-6 py-4">{order.formattedTotalPrice}</td>
              ...
              filteredTopCustomers.map((customer, index) => (
                <div key={customer._id || `customer-${index}`} className="flex items-center gap-4 pb-4 border-b last:border-b-0">
                  ...
                  <div className="text-right">
                    <p className="font-medium text-blue-600">{customer.formattedTotalSpent}</p>
                ...
                {filteredPaidOrders.length > 0 ? (
                  filteredPaidOrders.map((order, idx) => (
                    <tr key={order._id || `order-${idx}`} className="text-sm">
                      ...
                      <td className="px-6 py-4">{order.customerName || t('not_updated', 'common')}</td>
                      <td className="px-6 py-4">{order.formattedTotalPrice}</td>
                      <td className="px-6 py-4">{order.createdAt ? new Date(order.createdAt).toLocaleDateString(getIntlLocale(dashboardLocale)) : t('not_updated', 'common')}</td>
                    </tr>
                  ))
                ) : (
                  <tr>
