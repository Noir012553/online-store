...
import { formatCurrency, getImageUrl, isLoginPath } from "../../lib/utils";
...
export default function ProductDetail() {
  ...
  const { user } = useAuth();
  const { loadNamespace, t, locale, isLoadingNamespace } = useLanguage();
  ...
  useEffect(() => {
    if (!laptop || !translation || locale === 'vi') {
      return;
    }

    setLaptop((prev: any) => ({
      ...prev,
      ...(translation.name !== null && translation.name !== undefined && { name: translation.name }),
      ...(translation.description !== null && translation.description !== undefined && { description: translation.description }),
      ...(translation.specs !== undefined && { specs: translation.specs }),
      ...(translation.features !== undefined && { features: translation.features }),
      category: translation.categoryName ? { ...prev.category, name: translation.categoryName } : prev.category,
    }));
  }, [translation, locale]);
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      try {
        ...
        if (categoryId) {
          const allProducts = await productAPI.getProducts(1, undefined, categoryId, undefined, 4);
  ...
  if (error || !laptop) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <h2>{t('error_no_products_found')}</h2>
        {error && <p className="text-red-600 mt-2">{error}</p>}
        <Link href="/products">
          <Button className="mt-4 bg-red-600 hover:bg-red-700">{t('error_back_to_products')}</Button>
        </Link>
      </div>
    );
  }
  ...
  const convertedLaptop: Laptop = {
    id: laptop._id,
    name: laptop.name || '',
    brand: laptop.brand === 'Không thương hiệu' ? (locale === 'vi' ? 'Không thương hiệu' : 'No Brand') : t(laptop.brand, 'products'),
  ...
  return (
    <div className="container mx-auto px-4 py-8 animate-in fade-in duration-300">
      <Breadcrumbs
        links={[
          { label: t('breadcrumb_products'), href: "/products" },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
            ...
            {mainImage ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-gray-200 text-gray-500">
                <div className="text-center">
                  <p className="text-sm">{t('image_no_image_available')}</p>
                  {process.env.NODE_ENV === 'development' && (
                    <p className="text-xs mt-2 text-gray-400">
                      Debug: image={laptop?.image || 'null'}, images count={laptop?.images?.length || 0}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-4">
            ...
            <span className="text-xs sm:text-sm text-gray-500">({Math.max(totalReviews, reviews.length, laptop.numReviews || 0)} reviews)</span>
            <Badge variant={(laptop.countInStock || 0) > 0 ? "default" : "destructive"} className="text-xs sm:text-sm">
              {(laptop.countInStock || 0) > 0 ? `${t('filter_in_stock')} (${laptop.countInStock})` : t('out_of_stock')}
            </Badge>
          </div>
          ...
          <div className="flex flex-col gap-2 mb-4 sm:mb-6">
            ...
            <span className="text-2xl sm:text-3xl text-green-600 font-bold">{formatCurrency(laptop.price)}</span>
          </div>

          {Object.keys(laptop.specs || {}).length > 0 && (
            <div className="bg-white p-3 sm:p-4 rounded-lg mb-4 sm:mb-6">
              <h3 className="mb-2 sm:mb-3 text-sm sm:text-base font-semibold">{t('section_specifications')}</h3>
              <div className="space-y-1 sm:space-y-2 text-gray-700">
                {Object.entries(laptop.specs).slice(0, 5).map(([key, value]) => (
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 mb-4 sm:mb-6">
            ...
            >
              ...
              <span className="hidden sm:inline">{t('btn_add_to_cart')}</span>
              <span className="sm:hidden">{t('btn_add_mobile')}</span>
            </Button>
            <Button
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="mb-8 sm:mb-12">
        <TabsList className="grid w-full grid-cols-3 text-xs sm:text-sm">
          <TabsTrigger value="specs" className="text-xs sm:text-sm">{t('tab_specs')}</TabsTrigger>
          <TabsTrigger value="description" className="text-xs sm:text-sm">{t('tab_description')}</TabsTrigger>
          <TabsTrigger value="reviews" className="text-xs sm:text-sm">{t('tab_reviews')} ({Math.max(totalReviews, reviews.length, laptop.numReviews || 0)})</TabsTrigger>
        </TabsList>
        <TabsContent value="specs" id="product-specs-container" className="bg-white p-4 sm:p-6 border rounded-lg">
          <SpecsTable specs={laptop.specs} />
        </TabsContent>
        <TabsContent value="description" id="product-description-container" className="bg-white p-4 sm:p-6 border rounded-lg">
          <div className="space-y-8">
            {convertedLaptop.description && (
              <div>
                <h3 className="text-lg font-bold mb-4 text-gray-900">{t('section_description')}</h3>
            ...
            {(() => {
              ...
              const featureTranslations: { [key: string]: { vi: string; en: string } } = {
                'Thương hiệu chính hãng': { vi: 'Thương hiệu chính hãng', en: 'Genuine Brand' },
                'Bảo hành chính hãng 24 tháng': { vi: 'Bảo hành chính hãng 24 tháng', en: '24-Month Genuine Warranty' },
                'Hộ trợ đổi mới trong 7 ngày': { vi: 'Hộ trợ đổi mới trong 7 ngày', en: '7-Day Exchange Support' },
                'Hỗ trợ đổi mới trong 7 ngày': { vi: 'Hỗ trợ đổi mới trong 7 ngày', en: '7-Day Exchange Support' },
                'Giao hàng nhanh chóng': { vi: 'Giao hàng nhanh chóng', en: 'Fast Shipping' },
                'Hộ trợ khách hàng 24/7': { vi: 'Hộ trợ khách hàng 24/7', en: '24/7 Customer Support' },
                'Hỗ trợ khách hàng 24/7': { vi: 'Hỗ trợ khách hàng 24/7', en: '24/7 Customer Support' },
                'Kết nối không dây tiên lợi': { vi: 'Kết nối không dây tiên lợi', en: 'Wireless Connectivity' },
                'Kết nối không dây tiêu lời': { vi: 'Kết nối không dây tiêu lời', en: 'Extended Wireless Range' },
                'Kết nối không dây tiêu lớn': { vi: 'Kết nối không dây tiêu lớn', en: 'Extended Wireless Range' },
                'Đèn RGB/Nền có thể': { vi: 'Đèn RGB/Nền có thể', en: 'RGB Backlight Available' },
                'Thiết kế gọn gàng nhe': { vi: 'Thiết kế gọn gàng nhe', en: 'Compact & Elegant Design' },
                'Thời lượng pin lâu': { vi: 'Thời lượng pin lâu', en: 'Extended Battery Life' },
                'Tối ưu cho gaming': { vi: 'Tối ưu cho gaming', en: 'Gaming Optimized' },
                'Chống nước chống bụi': { vi: 'Chống nước chống bụi', en: 'Waterproof & Dustproof' },
                'Bàn phím cơ cao cấp': { vi: 'Bàn phím cơ cao cấp', en: 'Premium Mechanical Keyboard' },
                'Phù hợp dùng văn phòng': { vi: 'Phù hợp dùng văn phòng', en: 'Office Professional' },
                'Cảm biến độ chính xác cao': { vi: 'Cảm biến độ chính xác cao', en: 'High Precision Sensor' },
                'Thiết kế ergonomic thoải mái': { vi: 'Thiết kế ergonomic thoải mái', en: 'Ergonomic Design' },
                'Lập trình nút tùy chỉnh': { vi: 'Lập trình nút tùy chỉnh', en: 'Programmable Keys' },
                'Hiệu năng cao': { vi: 'Hiệu năng cao', en: 'High Performance' },
                'Chất lượng âm tuyệt vời': { vi: 'Chất lượng âm tuyệt vời', en: 'Superior Sound Quality' },
                'Kết nối ổn định': { vi: 'Kết nối ổn định', en: 'Stable Connection' },
              };
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {features.map((feature: string, index: number) => (
                      <li key={index} className="flex items-start gap-3 p-3 bg-white rounded-lg border border-red-100 hover:shadow-md transition-shadow">
                        ...
                        <span className="text-gray-800 font-medium">{getFeatureText(feature)}</span>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <div className="space-y-6">
              ...
              user ? (
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <p className="text-sm text-blue-800">{t('msg_login_to_review')}</p>
                  </div>
                  <Link href={loginHref}>
            ...
            {showReviewForm && (
              <div className="border p-6 rounded-lg bg-white space-y-4 mb-6">
                <h3 className="font-bold">{t('btn_write_review')}</h3>

                <div>
                  <label className="block text-sm mb-2">{t('label_rating')}</label>
                ...
                <div>
                  <label className="block text-sm mb-2">{t('label_your_review')}</label>
                  <textarea
                    id="review-comment"
                    name="comment"
                ...
                <div>
                  <label htmlFor="review-avatar" className="block text-sm mb-2">{t('label_avatar')}</label>
                  <div className="relative">
                    ...
                    <div className="w-full border rounded-lg p-3 bg-white hover:bg-white transition-colors cursor-pointer flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm">{reviewForm.avatar ? reviewForm.avatar.name : t('placeholder_no_file')}</span>
                    </div>
                  </div>
                </div>
            ...
            {reviews.length > 0 ? (
              reviews.map((review) => {
                const reviewerName = review.name || review.user?.name || t('default_anonymous');
                const reviewDate = review.createdAt ? new Date(review.createdAt).toLocaleDateString('vi-VN') : t('not_available', 'common');
              ...
              <p className="text-gray-500">{t('empty_no_reviews')}</p>
            )}
          </div>
        </TabsContent>
      </Tabs>
      ...
      {relatedLaptops.length > 0 && (
        <section>
          <h2 className="mb-4 sm:mb-6 text-lg sm:text-xl">{t('section_related_products')}</h2>
