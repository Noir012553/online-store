...
export default function CategoryProductsPage() {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="text-center py-12">
          <h1 className="text-3xl font-bold mb-4">{t('category_not_found_title', 'products')}</h1>
          <p className="text-gray-600 mb-6">{t('category_not_found_desc', 'products')}</p>
          <a
            href="/products"
            className="inline-block bg-red-600 text-white px-6 py-2 rounded hover:bg-red-700"
          >
            {t('back_to_products', 'products')}
          </a>
        </div>
      </div>
    );
  ...
  const translatedCategoryName = getCategoryName(selectedCategory);
