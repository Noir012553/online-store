...
- `online-store-backend/package.json` hiện chỉ giữ các script seed hợp lệ đã được kiểm tra trong báo cáo.
- `testRegistry.js` có 8 suite; các file được registry tham chiếu đều tồn tại.
- `resolveTestFiles()` khử trùng đường dẫn khi nhiều suite cùng tham chiếu một test.
- `online-store-frontend/src/test/offline-manual.js` đã dùng lệnh `node src/test/offline-manual.js`.
