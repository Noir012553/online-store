...
- UI dynamic vẫn tải status theo sản phẩm của trang hiện tại, nên bộ lọc status chưa đại diện toàn bộ catalog.
- `ProductCatalogTranslationCache` vẫn là nguồn ưu tiên; `LiveTranslationCache` vẫn là fallback cho dữ liệu/status legacy.
- `manualFields` vẫn được giữ trong luồng save và được loại khỏi field re-translate mặc định.
- Import/export vẫn giữ `productId` khi có, nhưng còn fallback `name + brand` khi thiếu định danh ổn định.
- Batch legacy vẫn nhận `lang`, `limit`, `entityType`, chưa hỗ trợ contract theo product/language/field.
