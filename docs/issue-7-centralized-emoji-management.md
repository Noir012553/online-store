...
- **Đã xác định:** `npm run check:emoji` hiện vẫn dừng ở chín file có ký hiệu hard-code tồn tại trước đó (`controller`, `i18n`, `seed`, `test`, `service` và script ngoài nhóm vừa chuẩn hóa). Không thay đổi chúng vì có thể liên quan API response hoặc dữ liệu nghiệp vụ và cần rà soát riêng.
- **Chưa thực hiện:** rà soát/chuẩn hóa toàn bộ các script diagnostic còn lại, các seeder còn lại và xác nhận ngoại lệ locale với người duy trì nội dung.
