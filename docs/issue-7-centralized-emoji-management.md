...
- **Trạng thái hiện tại:** Đã chuẩn hóa module emoji UI và ký hiệu CLI; đã bổ sung kiểm tra tĩnh; locale và nội dung gửi người dùng được phân loại là ngoại lệ có chủ đích
...
- **Đã hoàn thành:** rà soát và chuẩn hóa nhóm script diagnostic chỉ đọc còn lại trong phạm vi an toàn, gồm health check i18n, chẩn đoán schema, kiểm tra cấu trúc bản dịch và test phase 4 giản lược; các file đã được đưa vào enforcement.
- **Đã hoàn thành:** phân loại các ngoại lệ không thuộc ký hiệu CLI: emoji trong API debug response, email marketing, locale, metadata cờ ngôn ngữ, report lưu file, test có side effect và script legacy; giữ nguyên nội dung để không thay đổi contract hoặc dữ liệu đã biên tập.
- **Đã hoàn thành:** kiểm tra tĩnh hiện có đáp ứng tiêu chí ngăn emoji CLI hard-code trong các runtime entry point thuộc allowlist. Repository chưa có workflow CI, vì vậy chưa tạo pipeline mới ngoài phạm vi issue; lệnh `check:emoji` là bước kiểm tra chính thức dùng trong review/chạy thủ công.

Không thay đổi emoji trong locale, test fixture có side effect, report lưu file, API response hoặc email template vì đây là nội dung/contract cần quy trình riêng.

**Trạng thái cuối:** Đã hoàn thành mục tiêu quản lý tập trung emoji UI và ký hiệu CLI trong phạm vi runtime an toàn được xác định. Frontend production build, backend kiểm tra tĩnh, kiểm tra cú pháp JavaScript và các kiểm tra format đều đã được xác thực. Các vị trí còn hard-code đều đã được phân loại thành ngoại lệ có chủ đích hoặc phạm vi cần quy trình nội dung/API riêng.
...
- **Thấp — nhóm test backend ngoài enforcement:** `online-store-backend/src/test/test-phase4-e2e.js` và `test-rollback-procedures.js` vẫn dùng emoji/ký hiệu Unicode trực tiếp trong test có thao tác ghi/xóa dữ liệu. Các file này được giữ ngoài allowlist vì việc thay đổi cần rà soát riêng về side effect và nội dung test; các test diagnostic chỉ đọc còn lại đã được chuẩn hóa hoặc ghi nhận là ngoại lệ.
...
- **Đã hoàn thành thêm:** chuẩn hóa ký hiệu output của `src/scripts/performance-benchmark.js` qua `CLI_SYMBOLS`, bao gồm prefix log, hướng cải thiện, summary và mũi tên so sánh; giữ nguyên output terminal.
...
### Phát hiện rà soát bổ sung — ký hiệu terminal ngoài registry

- **Trung bình — `online-store-frontend/src/test/offline-manual.js:42, 62, 77, 92, 100, 112-114, 128-129, 141, 157, 162-175, 178, 185-187, 235-245`:** file đã dùng registry `src/test/cliSymbols.js` cho `✓`, `✗`, `ℹ`, `⚠`, nhưng vẫn hard-code `└─`, `→` và các dòng phân cách `════════` trong output test/CLI. Phát hiện này bổ sung cho việc file đã được ghi nhận ở mục trước; checker frontend hiện chỉ dò dải emoji `\u1F000-\u1FAFF` và `\u2600-\u27BF`, nên không phát hiện box-drawing hoặc mũi tên. Cần quyết định có đưa các ký hiệu terminal này vào registry hay ghi rõ chúng là format riêng của test.
- **Trung bình — `online-store-backend/src/test/test-backend-endpoints-phase3.js:54, 70, 99, 114, 143, 167, 172, 189, 225, 245, 250, 263, 282, 293, 303, 316-318, 357-359, 367`:** test backend độc lập còn hard-code `└─`, `→`, `════════` và `✓` trong log, nhưng không có trong `checkedFiles` của `scripts/check-cli-symbols.js`. Vì vậy `npm run check:emoji` không bao phủ suite này dù đây là runtime test/CLI có output terminal; cần xác minh suite còn được vận hành trước khi thêm vào allowlist và chuẩn hóa qua `CLI_SYMBOLS`.
...
### Phát hiện rà soát bổ sung — đối soát checker hiện tại

- **Trung bình — `online-store-backend/src/seeds/translationSeeder.js:119`:** nhánh lỗi của `seedTranslations` vẫn ghi trực tiếp `❌` qua `console.error('❌ Translation seeding failed:', error)`. File không nằm trong `checkedFiles` tại `online-store-backend/scripts/check-cli-symbols.js:5-92`, nên `npm run check:emoji` backend vẫn pass mà không phát hiện. Đây là output terminal của seeder runtime, phù hợp để rà soát riêng việc chuyển sang `CLI_SYMBOLS.error`; không nên coi kết quả pass hiện tại là đã bao phủ entry point này.
- **Trung bình — `online-store-backend/scripts/check-cli-symbols.js:93-100`:** checker chỉ phát hiện ký hiệu Unicode nếu ký hiệu nằm trên đúng dòng có lời gọi `console.log`, `console.warn`, `console.error`, `console.time` hoặc `console.timeEnd`, đồng thời chỉ quét allowlist tĩnh. Vì vậy checker không thể bao phủ ký hiệu được tạo qua biến/template nhiều dòng, wrapper logger, `throw new Error(...)`, nội dung ghi file hoặc mọi entry point ngoài allowlist. Lệnh backend đã được chạy lại và pass, nhưng kết quả này chỉ xác nhận phạm vi hẹp nói trên.
...
### Phát hiện rà soát bổ sung — suite i18n được đăng ký nhưng ngoài enforcement

- **Trung bình — `online-store-backend/src/test/test-languages-flow.js:52-54`:** test tạo chuỗi trạng thái `✅ Added` hoặc `❌ Not added` trực tiếp trong biến `status`. File này không nằm trong `checkedFiles` tại `online-store-backend/scripts/check-cli-symbols.js:5-92`, nhưng lại được đăng ký trong suite `i18n` của test runner (`online-store-backend/src/test/testRegistry.js:19-28`), nên có thể được thực thi qua `npm test -- --suite=i18n`. Vì checker hiện chỉ quét allowlist tĩnh và các dòng gọi `console.*`, `npm run check:emoji` không phát hiện literal này. Cần quyết định hoặc đưa test runtime này vào registry/checker khi hoàn thiện output của nó, hoặc loại bỏ chuỗi/trạng thái không được dùng; không xem đây là test legacy trước khi rà soát luồng i18n.

**Cập nhật bước tiếp theo:** rà soát các file được `TEST_SUITES` đăng ký để đối chiếu với allowlist checker, ưu tiên suite i18n vì `test-languages-flow.js` là entry point có thể chạy qua lệnh test chính thức nhưng hiện không được coverage bởi quy ước emoji CLI.

### Phát hiện rà soát bổ sung — Unicode trong tiêu đề test đã được đăng ký
