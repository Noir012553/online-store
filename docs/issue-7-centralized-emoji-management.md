...
- **Trạng thái hiện tại:** Đã phân tích; chưa chỉnh sửa mã nguồn, dữ liệu hoặc cấu hình
...
- **Chưa thực hiện:** tạo constants, thay thế hard-code, chỉnh locale hoặc thêm kiểm tra CI.

Bước tiếp theo nên là hoàn tất bảng kiểm kê theo từng file, sau đó chốt danh sách emoji chuẩn và danh sách ngoại lệ trước khi sửa code. Không nên bắt đầu thay thế hàng loạt khi chưa phân biệt xong emoji giao diện, prefix log và nội dung dịch.

**Trạng thái cuối:** Đã lập kế hoạch quản lý emoji tập trung; chưa triển khai thay đổi mã nguồn.
