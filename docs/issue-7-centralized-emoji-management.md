...
- **Chưa thực hiện:** kiểm kê đầy đủ theo từng file, xác nhận ngoại lệ locale với người duy trì nội dung, cài dependencies frontend để chạy `check:emoji`/type check và tích hợp kiểm tra vào CI.

Bước tiếp theo là cài dependencies frontend trong môi trường phù hợp, chạy kiểm tra build/type check frontend và quyết định danh sách ngoại lệ locale trước khi mở rộng quy tắc quét. Không thay đổi emoji trong locale nếu chưa có xác nhận về nội dung biên tập.

**Trạng thái cuối:** Đã triển khai quản lý tập trung cho emoji UI và ký hiệu CLI trong phạm vi runtime đã chọn; locale vẫn được giữ nguyên.
