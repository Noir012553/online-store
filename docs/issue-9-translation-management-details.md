# Issue 9 — Chi tiết kỹ thuật và blocker translation sản phẩm

> Tài liệu này chứa phần rà soát chi tiết được tách từ issue-8. Tóm tắt quyết định và trạng thái hiện tại xem tại `docs/issue-8-dynamic-product-translation-management.md`.
...
## Cập nhật: Export hiển thị key feature thay vì nhãn dịch

### Hiện tượng

File JSON/CSV export có trường `features` chứa các mã như `feature_genuine_brand`, `feature_wireless_connectivity` thay vì nhãn hiển thị bằng ngôn ngữ đang chọn. Khi import lại file nguyên trạng, kết quả `0 sản phẩm mới, 0 cập nhật` là bình thường nếu không có trường nào thay đổi; bản ghi được tính là `unchanged`.

### Quyết định xử lý

- Giữ `features` là mã định danh gốc để import/export round-trip không làm mất khả năng dịch đa ngôn ngữ.
- Bổ sung `featureLabels` vào JSON và CSV export. Trường này chứa nhãn static đã dịch theo query `lang` hoặc ngôn ngữ request, chỉ phục vụ đọc/đối soát và không được dùng thay cho `features`.
- Import tiếp tục bỏ qua metadata `featureLabels`, nên file export có thể nhập lại mà không ghi nhãn dịch vào dữ liệu nguồn.
- Không tự động đưa bản dịch dynamic hoặc `featuresTranslations` vào export sản phẩm nguồn; backup đa ngôn ngữ vẫn là phạm vi riêng.

### Đã triển khai

- `online-store-backend/src/controllers/productImportController.js` bổ sung ánh xạ nhãn feature qua namespace `products` và thêm `featureLabels` cho JSON/CSV.
- Hỗ trợ `GET /api/admin/products/export?lang=en` để chọn ngôn ngữ nhãn export; nếu không truyền sẽ dùng ngôn ngữ request hoặc mặc định `vi`.
