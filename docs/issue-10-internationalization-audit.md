...
- Rate-limit middleware đã trả `code`, `retryAfter` và message qua `getMessage(req.lang, ...)`.
- Validation middleware đã trả mã `VALIDATION_FAILED` và message theo locale.
- `RoleBadge` đã dùng key i18n cho label và description.
- Helper phân quyền chứa literal tiếng Anh đã được xóa vì không còn consumer.
- Placeholder ký hiệu tiền tệ được giữ nguyên vì là dữ liệu minh họa, không phải câu thông báo cần dịch.
