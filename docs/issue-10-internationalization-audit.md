...
Đợt cập nhật này chỉ bổ sung trạng thái từ rà soát tài liệu và mã nguồn; chưa thay đổi contract API, chưa thêm khóa dịch và chưa chạy kiểm thử đa locale.

### Đã xử lý hoặc xác minh không còn là lỗi mở

- [x] Rate-limit middleware đã trả `code`, `retryAfter` và message qua `getMessage(req.lang, ...)`.
- [x] Validation middleware đã trả mã `VALIDATION_FAILED` và message theo locale.
- [x] `RoleBadge` đã dùng key i18n cho label và description.
- [x] Helper phân quyền chứa literal tiếng Anh đã được xóa vì không còn consumer.
- [x] Placeholder ký hiệu tiền tệ được giữ nguyên vì là dữ liệu minh họa, không phải câu thông báo cần dịch.

### Nhóm còn mở, cần ưu tiên

1. Checkout/order/shipping/payment: chuyển từ so khớp `Error.message` sang `code` và `params` ổn định.
2. Translation controller: chuẩn hóa các response/error literal sang `getMessage()` và mã lỗi.
3. Currency/exchange-rate và product validation: trả code/key từ service/model, dịch tại boundary có `req.lang`.
4. Import/export và email: loại bỏ raw provider error, dịch text fallback và status có thể trả về API.
5. Admin/debug endpoints: quyết định endpoint nào internal-only; endpoint còn hiển thị trong UI phải có contract locale rõ ràng.

### Chưa thể đóng audit

Chưa có kiểm thử cùng một error path với `vi`, `en` và locale không-Latin; cũng chưa có kiểm thử xác nhận frontend không còn phụ thuộc `message.includes(...)`. Vì vậy audit vẫn ở trạng thái **chưa hoàn tất**.

### Bước tiếp theo

Chốt contract lỗi chung `{ code, params, message }`, triển khai theo từng luồng ưu tiên và kiểm thử cả success/error path trước khi cập nhật trạng thái đóng issue.
...
- Lựa chọn vận chuyển không hợp lệ khi tạo đơn đã dùng `ORDER_SHIPPING_SELECTION_INVALID` và thông điệp theo locale thay cho literal tiếng Anh. Dịch vụ vận chuyển trả mã `SHIPPING_PROVIDER_UNAVAILABLE` hoặc `SHIPPING_SERVICE_UNAVAILABLE`; `orderController` giữ nguyên mã này trong response lỗi nhưng không lộ thông báo nội bộ.
- Đã kiểm tra cú pháp `orderController.js`, `shippingService.js`, `npm run check:emoji` tại backend và `git diff --check`: **đạt**. Chưa chạy kiểm thử tích hợp tạo đơn vì môi trường hiện chưa có MongoDB/JWT/Cloudflare hợp lệ.
- Đã xóa helper phân quyền không sử dụng chứa literal tiếng Anh; `RoleBadge` tiếp tục dùng key dịch cho cả nhãn và tooltip.

### Còn lại tại thời điểm cập nhật

- Chuẩn hóa contract lỗi và kiểm thử API riêng cho các nhóm payment, translation, currency, import/export và email.
- Rà soát các lỗi provider/raw `error.message` còn có thể đi ra API ở những nhóm trên; không thay đổi hàng loạt khi chưa xác định caller và locale.
- Chạy đầy đủ backend test trong môi trường có MongoDB, JWT và Cloudflare credentials hợp lệ.

Các nhóm validation, rate-limit, order currency, shipping, user, Cloudinary, banner translation và các fallback frontend đã được ghi nhận là hoàn tất ở các mục cập nhật sau trong tài liệu này.
...
- Sau khi cài đúng dependencies theo `package.json` mà không tạo lockfile, `npm run build` và `npm run check:emoji` tại frontend đều **đạt**.
- Backend `npm run check:emoji` và các kiểm tra cú pháp liên quan cũng **đạt**; kiểm thử tích hợp backend đầy đủ vẫn cần MongoDB, JWT và Cloudflare credentials hợp lệ.

## Cập nhật rà soát và xử lý tiếp theo

- Đã đối chiếu `BannerTranslationManager.tsx`: các nội dung coverage, empty state và danh sách ngôn ngữ thiếu đều gọi key i18n trong namespace `admin-banners`; không còn fallback tiếng Việt render trực tiếp tại các vị trí đã nêu ở A1.
- Đã kiểm tra `language_coverage`, `click_add_translation_to_start` và `missing_translations` trong toàn bộ locale có `admin-banners.json`; tất cả key đều tồn tại và có giá trị không rỗng.
- Đã xử lý fallback lỗi tạo đánh giá tại `online-store-frontend/src/pages/product/[id].tsx`: API layer trả mã ổn định `REVIEW_CREATION_FAILED` trong thuộc tính `code` khi backend không gửi message; toast hiển thị `products.review_error` theo locale thay vì lộ key thô. Thông báo cụ thể do backend gửi vẫn được giữ nguyên.
- `rateLimitMiddleware.js` đã được kiểm tra lại: tất cả limiter trả `code`, message qua `getMessage(req.lang, 'errors.too_many_requests_title')` và `retryAfter`; mục B1 trong phần phát hiện bổ sung được xem là đã xử lý.
- Các hạng mục còn lại tập trung ở contract error có cấu trúc cho payment, translation, currency, import/export và email; không tự thay đổi hàng loạt vì các response này cần kiểm thử API theo từng luồng.

### Cập nhật xử lý Cloudinary

- Đã chuẩn hóa response lỗi tại `online-store-backend/src/controllers/cloudinaryController.js` theo cấu trúc `success`, `code`, `error` và `message`.
- Các lỗi folder upload, metadata, kích thước, định dạng ảnh, tạo chữ ký và xác thực ảnh đã dùng `getMessage(req.lang, 'common.*')`; không còn trả literal tiếng Anh hoặc `error.message` nội bộ trực tiếp về client.
- Các mã lỗi ổn định mới gồm `UPLOAD_FOLDER_INVALID`, `UPLOAD_SIGNATURE_FAILED`, `IMAGE_METADATA_INVALID`, `IMAGE_DIMENSIONS_INVALID`, `UPLOAD_FILE_TOO_LARGE`, `IMAGE_FORMAT_INVALID` và `IMAGE_VALIDATION_REQUEST_FAILED`.
- Đã xác minh `node --check src/controllers/cloudinaryController.js`, `npm run check:emoji` và `git diff --check`: **đạt**.
- Đã xác minh sáu khóa Cloudinary hiện có cho `vi` và `en`; toàn bộ đều trả về chuỗi dịch hợp lệ.

**Trạng thái cập nhật:** Nhóm lỗi upload/xác thực Cloudinary trong mục B11 đã được xử lý. Các nhóm payment, shipment, translation, user, currency, import/export và email vẫn cần triển khai và kiểm thử API riêng.

### Cập nhật chuẩn hóa lỗi frontend

- `online-store-frontend/src/lib/translationService.ts` đã dùng `TranslationServiceError` có `code` và `params` cho validation ngôn ngữ và các lỗi request/response của dịch trực tiếp; không còn ném câu tiếng Anh hoặc i18n key thô ở các nhánh này.
- `online-store-frontend/src/lib/api.ts` trả mã `REVIEW_CREATION_FAILED` có cấu trúc khi tạo review thất bại mà backend không gửi message.
- `online-store-frontend/src/pages/product/[id].tsx` đọc `error.code` để chọn bản dịch fallback, không phụ thuộc vào nội dung `Error.message`.
- Đã cài đúng dependency frontend theo `package.json` mà không tạo lockfile. `npm test` đạt **10/10**, `npm run check:emoji` đạt và `npm run build` hoàn tất thành công.
- `npx tsc --noEmit` còn lỗi ở `src/test/offline-support.test.ts` vì thiếu khai báo kiểu của Jest/Mocha (`describe`, `test`, `expect`); đây là cấu hình test có sẵn, không xuất hiện trong TypeScript check của `next build` và không thuộc phần mã vừa cập nhật.

**Trạng thái cập nhật mới nhất:** Các fallback i18n frontend trong phạm vi này đã được chuẩn hóa theo mã lỗi. Các endpoint backend còn lại vẫn cần được xử lý từng luồng và kiểm thử API riêng.

### Cập nhật i18n thao tác bản dịch banner

- `online-store-backend/src/controllers/bannerController.js` đã dùng `getMessage(lang, 'api-errors.banner_translation_not_found')` cho cả nhánh cập nhật và xóa khi không tìm thấy bản dịch banner.
- Response xóa bản dịch banner hiện dùng `getMessage(lang, 'api-errors.banner_translation_deleted')`; không còn trả literal tiếng Anh `Translation deleted`.
- Đã xác minh hai khóa i18n trên có giá trị tại toàn bộ **9 locale** hiện có.
- Đã chạy `node --check src/controllers/bannerController.js` và `git diff --check`: **đạt**.

**Trạng thái cập nhật:** Nhóm banner CRUD trong mục B9 đã được xử lý. Các nhóm backend còn lại gồm payment, translation, user, currency, import/export và email vẫn cần chuẩn hóa contract lỗi và kiểm thử API riêng.

### Đối chiếu tiến độ với commit hiện tại

Đã đối chiếu nội dung báo cáo với lịch sử commit tại commit hiện tại `2f698c2`:

- Các thay đổi chuẩn hóa lỗi frontend, user, shipment và banner translation đã được tích hợp qua các commit `771dfcb`, `faa2fef`, `5e28f31` và `27d1f8c`.
- Các hạng mục tương ứng trong phần tiến độ của báo cáo đã phản ánh đúng phạm vi đã hoàn thành; không phát hiện chênh lệch cần sửa mã nguồn.
