...
- Đã xử lý fallback lỗi tạo đánh giá tại `online-store-frontend/src/pages/product/[id].tsx`: nếu API không gửi message và API layer trả key nội bộ `review_creation_failed`, toast nay hiển thị `products.review_error` theo locale thay vì lộ key thô. Thông báo cụ thể do backend gửi vẫn được giữ nguyên.
