...
- **Đã xác nhận trong mã nguồn:** Registry và test runner vẫn còn các suite rollback, shadow-write, simple, orders và VNPay; các test này chưa thể coi là dư thừa.
- **Còn theo dõi:** Cần môi trường MongoDB, JWT và Cloudflare hợp lệ để xác minh runtime/fixture; không có kết quả chạy mới trong đợt đối chiếu tài liệu này.
