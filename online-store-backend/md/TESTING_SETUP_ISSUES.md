...
Integration suite dùng các biến môi trường sau: `TEST_MONGO_URI` hoặc `MONGO_URI` trỏ tới MongoDB test, `TEST_ADMIN_EMAIL` và `TEST_ADMIN_PASSWORD` cho tài khoản admin local, `JWT_ACCESS_SECRET` và `JWT_REFRESH_SECRET` cho JWT, cùng `TEST_BASE_URL`/`TEST_API_BASE_URL` cho endpoint. Nếu dùng backend đã chạy sẵn, bắt buộc có `TEST_MONGO_URI` để tránh ghi vào database thật; có thể dùng `TEST_ADMIN_TOKEN`, còn nếu không thì harness sẽ tạo tài khoản admin test khi cần và đăng nhập qua `POST /api/users/login`. Nếu backend chưa chạy, harness tự khởi động process riêng với database cô lập từ `MONGO_URI`. Không đặt secret hoặc mật khẩu trong source code; file mẫu là `.env.example`.
