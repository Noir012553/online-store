...
def get_all_collection_urls():
    ...
    page = 1
    while True:
        # URL danh mục Laptop Acer học tập và làm việc
        url = f"https://gearvn.com/collections/laptop-acer-hoc-tap-va-lam-viec?page={page}"
        ...
        try:
            ...
            # Nếu không tìm thấy link nào mới trong trang này, dừng lại
            if not found_any: 
                break
            page += 1
            time.sleep(0.5)
...
def scrape_full():
    print(">>> Bắt đầu quét danh sách sản phẩm Acer (Học tập & Làm việc)...")
    ...
    file_prefix = f"Acer_Laptop_Office_{date_str}"
    ...
    print(f"\n>>> Hoàn thành! Đã xuất 38 sản phẩm thành công:")
