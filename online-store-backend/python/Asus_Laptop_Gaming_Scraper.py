...
def get_all_collection_urls():
    ...
    page = 1
    while True:
        # URL danh mục Laptop Gaming Asus
        url = f"https://gearvn.com/collections/laptop-gaming-asus?page={page}"
...
def scrape_full():
    ...
    for url in product_urls:
        ...
        try:
            ...
            # --- DỌN DẸP RÁC (Loại bỏ Sản phẩm tương tự, Sản phẩm đã xem và Mua kèm giá sốc) ---
    ...
    # --- TẠO TÊN FILE ĐỘNG THEO CHUẨN Brand_Categories_time ---
    brand_name = "Asus"
    category_name = "Laptop_Gaming"
    date_str = datetime.datetime.now().strftime("%Y%m%d")
    
    file_prefix = f"{brand_name}_{category_name}_{date_str}"
