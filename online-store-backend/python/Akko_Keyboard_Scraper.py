...
def get_all_collection_urls():
  ...
  page = 1
  while True:
    # URL danh mục Bàn phím Akko trên GearVN
    url = f'https://gearvn.com/collections/ban-phim-akko?page={page}'
...
def scrape_full():
  print('>>> Bắt đầu quét danh sách Bàn phím Akko...')
    ...
    try:
      ...
      if json_ld:
        try:
          ...
          instock = (
              'In Stock'
              if item.get('offers', {}).get('availability')
  ...
  # Đã sửa lại định dạng tên file xuất ra theo quy tắc viết hoa gạch dưới
  file_prefix = f'Akko_Keyboard_{date_str}'
