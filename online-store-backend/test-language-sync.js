...
async function main() {
  ...
  await test('Verify LanguageService cache (check vi, en, pt)', async () => {
    const res = await axios.get(`${API_BASE}/languages`);
    const activeLangs = res.data.data.map(l => l.code);
    console.log(`   Active languages: ${activeLangs.join(', ')}`);
    if (!activeLangs.includes('vi') || !activeLangs.includes('en')) {
      throw new Error('VI or EN missing from active languages');
    }
  });
