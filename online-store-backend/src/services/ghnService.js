...
async function getGhnClient() {
  ...
  const shopId = process.env.GHN_SHOP_ID || provider.token;
  if (shopId) {
...
async function getDistricts(provinceId, lang) {
  ...
  if (!provinceId) {
    throw new Error(getMessage(ghnLang, 'shipping.provinceIdRequired'));
  }
  ...
  try {
    ...
    const msg = getMessage(ghnLang, 'shipping.ghnDistrictError');
...
async function getWards(districtId, lang) {
  ...
  if (!districtId) {
    throw new Error(getMessage(ghnLang, 'shipping.districtIdRequired'));
  }
  ...
  try {
    ...
    if (getResponse.data?.code === 200) {
      const wards = getWardList(getResponse.data);
    ...
    if (postResponse.data?.code === 200) return getWardList(postResponse.data);

    throw new Error(`GHN API error: ${postResponse.data?.message || getResponse.data?.message}`);
  } catch (error) {
    console.error('GHN getWards error:', error.message);
    const msg = getMessage(ghnLang, 'shipping.ghnWardError');
    throw new Error(msg.replace('{{error}}', error.message));
  }
}
    ...
    if (from_id === to_id) {
      ...
      return {
        ...
        data: {
          ...
          service_fee: 0,
          insurance_fee: 0,
          message: getMessage(ghnLang, 'shipping.sameDistrictFree'),
        },
      };
    }
    ...
    services: [
      {
        ...
        name: Object.fromEntries(
          activeLangs.map(lang => [
            lang,
            getMessage(lang.toUpperCase(), 'shipping.standardServiceName')
          ])
        ),
        estimatedDays: '2-3',
        description: Object.fromEntries(
          activeLangs.map(lang => [
            lang,
            getMessage(lang.toUpperCase(), 'shipping.standardServiceDesc')
          ])
        ),
      },
      {
        ...
        name: Object.fromEntries(
          activeLangs.map(lang => [
            lang,
            getMessage(lang.toUpperCase(), 'shipping.expressServiceName')
          ])
        ),
        estimatedDays: '1-2',
        description: Object.fromEntries(
          activeLangs.map(lang => [
            lang,
            getMessage(lang.toUpperCase(), 'shipping.expressServiceDesc')
          ])
        ),
      },
    ],
...
async function createShipment(params) {
  try {
    ...
    if (!feeResult.success) {
      ...
      return {
        success: false,
        error: getMessage(ghnLang, 'shipping.calculateFeeFailed').replace('{{error}}', feeResult.error),
      };
    }
