...
const sendVerificationEmail = async (email, verificationUrl) => {
  try {
    ...
    const msg = getMessage('VI', 'email.verification');
    const mailOptions = {
...
const sendResetPasswordEmail = async (email, resetUrl) => {
  try {
    ...
    const msg = getMessage('VI', 'email.resetPassword');
    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: email,
      subject: msg.subject,
...
const sendOTPEmail = async (email, otp) => {
  try {
    ...
    const msg = getMessage('VI', 'email.otp');
    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: email,
      subject: msg.subject,
...
const sendNewsletterConfirmationEmail = async (email) => {
  try {
    ...
    const msg = getMessage('VI', 'email.newsletter');
    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to: email,
      subject: msg.subject,
