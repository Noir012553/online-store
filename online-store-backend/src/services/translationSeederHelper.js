...
class TranslationSeederHelper {
  ...
  async translateWithRetry(text, targetLang, retries = 0) {
    ...
    try {
      ...
      try {
        const result = await cloudflareAiService.translate(text, 'vi', targetLang, controller.signal);
  ...
  /**
   * Batch save translations to cache
   */
  async batchSaveCache(records) {
   ...
   */
  async translateField(text, targetLang, entityId = null, entityType = 'generic', specKey = null) {
  ...
  /**
   * Translate category object (name, description)
   */
  async translateCategory(category, targetLang) {
    ...
    const promises = fieldsToTranslate.map(field =>
      this.translateField(
        ...
        targetLang,
        categoryId,
        field === 'name' ? 'category_name' : 'category_description'
      ).then(result => {
        results[field] = result;
      })
    );
  ...
  /**
   * Check if product already has English translations in database
   * INCREMENTAL: Skip if all translations exist
   */
  async isProductTranslated(productId, targetLang = 'en') {
    if (!this.config.INCREMENTAL) {
      return false; // Full seed: translate everything
    }
    ...
    try {
      ...
      return Boolean(product.translations[targetLang]);
    } catch (error) {
      console.warn(`  ⚠️ Failed to check translation status: ${error.message}`);
      return false;
    }
  }
   ...
   * targetLanguages: ['en'] (only English supported)
   ...
   */
  async translateProductsBatch(products, targetLanguages = ['en']) {
    const stats = {
      total: 0,
      cached: 0,
      translated: 0,
  ...
  /**
   * Check if category already has translations
   */
  async isCategoryTranslated(categoryId, targetLang = 'en') {
    if (!this.config.INCREMENTAL) {
      return false;
    }
    ...
    try {
      ...
      return Boolean(category.translations[targetLang]);
    } catch (error) {
   ...
   */
  async translateCategoriesBatch(categories, targetLanguages = ['en']) {
    const stats = {
      total: 0,
      cached: 0,
      translated: 0,
    ...
    if (this.config.INCREMENTAL) {
      ...
      const translatedCategories = await Category.find(
        { _id: { $in: categoryIds } },
        { _id: 1, translations: 1 }
      ).lean();
      ...
      translatedCategories.forEach(c => {
        translatedMap.set(String(c._id), c.translations || {});
      });

      for (const category of categories) {
        for (const targetLang of targetLanguages) {
          const categoryTranslations = translatedMap.get(String(category._id)) || {};
          if (!categoryTranslations[targetLang]) {
   ...
   */
  async translateReviewsBatch(reviews, targetLanguages = ['en']) {
    const stats = {
      total: 0,
      cached: 0,
      translated: 0,
