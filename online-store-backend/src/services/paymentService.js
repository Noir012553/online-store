...
class PaymentService {
  ...
  async confirmPayment(orderId) {
    try {
      ...
      // Payment amount must match order total (allow 100 VND tolerance for rounding)
      const amountDifference = Math.abs(payment.amount - order.totalPrice);
      const AMOUNT_TOLERANCE = 100; // VND
      ...
      if (amountDifference > AMOUNT_TOLERANCE) {
        ...
        return {
          success: false,
          error: `Payment amount (${payment.amount}) does not match order total (${order.totalPrice})`,
      ...
      return {
        ...
        data: {
          ...
          paidAmount: payment.amount,
          totalPrice: order.totalPrice,
          verified: true, // Confirms amount matched
        },
      };
    } catch (error) {
