...
const createBanner = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(400);
    throw new Error(getMessage(lang, 'common.image_validation_failed'));
  }
  ...
  try {
    ...
    try {
      await deleteFromCloudinary(imageData.imagePublicId);
    } catch (cleanupError) {
...
const updateBanner = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (error.name === 'VersionError') {
      res.status(409);
      throw new Error('Banner was changed by another administrator. Please reload and try again.');
    }
...
const hardDeleteBanner = asyncHandler(async (req, res) => {
  ...
  if (cleanupId) {
    await enqueueCloudinaryCleanup(cleanupId);
  }
