...
const getCoupons = asyncHandler(async (req, res) => {
  ...
  res.json({ coupons: await formatReportingCoupons(coupons, reportingCurrency, req.locale), page, pages: Math.ceil(count / pageSize), total: count });
});
...
const getCouponById = asyncHandler(async (req, res) => {
  ...
  if (coupon) {
    res.json((await formatCoupons([coupon], req.locale))[0]);
  } else {
    res.status(404);
...
const getDeletedCoupons = asyncHandler(async (req, res) => {
  ...
  res.json({ coupons: await formatCoupons(coupons, req.locale), page, pages: Math.ceil(count / pageSize), total: count });
});
...
const createCoupon = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (io) {
      const formattedCoupon = (await formatCoupons([createdCoupon], req.locale))[0];
      broadcastCouponCreated(io, {
        ...
        formattedDiscountValue: formattedCoupon.formattedDiscountValue,
        maxUses: createdCoupon.maxUses,
        minOrderAmount: createdCoupon.minOrderAmount,
        formattedMinOrderAmount: formattedCoupon.formattedMinOrderAmount,
  ...
  res.status(201).json((await formatCoupons([createdCoupon], req.locale))[0]);
});
...
const updateCoupon = asyncHandler(async (req, res) => {
  ...
  try {
    const io = req.app.get('io');
    if (io) {
      const formattedCoupon = (await formatCoupons([updatedCoupon], req.locale))[0];
  ...
  res.json((await formatCoupons([updatedCoupon], req.locale))[0]);
});
...
const restoreCoupon = asyncHandler(async (req, res) => {
  ...
  res.json({ message: getMessage(lang, 'admin-controllers-messages.coupon_restored'), coupon: (await formatCoupons([coupon], req.locale))[0] });
});
...
const calculateDiscount = asyncHandler(async (req, res) => {
  ...
  const couponCurrency = couponCurrencyCode === normalizedOrderCurrencyCode
    ? orderCurrency
    : await Currency.findOne(
      { code: couponCurrencyCode, isActive: true },
      { code: 1, symbol: 1, position: 1, decimalPlaces: 1, _id: 0 }
    ).lean();
  ...
  res.json({
    ...
    discountType: coupon.discountType,
    discountValue: coupon.discountValue,
    ...(coupon.discountType === 'fixed' && couponCurrency && {
      formattedDiscountValue: formatCurrency(coupon.discountValue, couponCurrency, lang),
    }),
    minimumOrderAmount,
    formattedMinOrderAmount: formatCurrency(minimumOrderAmount, orderCurrency, lang),
