...
const formatOrderResponse = async (orders, req) => {
  ...
  return formatReportingOrders(orders, reportingCurrency, req.locale);
};
...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  if (updated) {
    ...
    res.json((await formatOrders([updatedOrder], req.locale))[0]);
  } else {
    res.json((await formatOrders([order], req.locale))[0]);
  }
});
...
const addOrderItems = asyncHandler(async (req, res) => {
  ...
  if (!req.isSummaryRequest && idempotencyKey) {
    ...
    if (existingOrder) {
      ...
      return res.status(200).json({
        success: true,
        data: (await formatOrders([existingOrder], req.locale))[0],
        isDuplicate: true,
      });
    }
  }
  ...
  if (!req.isSummaryRequest && (customerPhone || customerName || customerEmail)) {
    try {
  ...
  if (shippingProvider || shippingService) {
    ...
    const totalWeight = Array.from(productMap.values()).reduce(
      (weight, { product, quantity }) => weight + (getProductWeightInGrams(product) * quantity),
      0
    );
  ...
  try {
    const io = req.app.get('io');
    if (io) {
      const formattedOrder = (await formatOrders([populatedOrder], req.locale))[0];
      broadcastNewOrder(io, {
        ...
        formattedTotalPrice: formattedOrder.formattedTotalPrice,
        currencyCode: populatedOrder.currencyCode,
  ...
  res.status(201).json({
    success: true,
    data: (await formatOrders([populatedOrder], req.locale))[0],
  });
});
...
const getMyOrders = asyncHandler(async (req, res) => {
  ...
  const totalsByCurrency = orders.reduce((totals, order) => {
    ...
    if (!currencyCode || !Number.isFinite(order.totalPrice)) return totals;
    totals[currencyCode] = (totals[currencyCode] || 0) + order.totalPrice;
    return totals;
  }, {});
  const formattedSummary = await formatOrders(
    Object.entries(totalsByCurrency).map(([currencyCode, totalPrice]) => ({
      currencyCode,
      totalPrice,
      orderItems: [],
    })),
  ...
  res.json({
    ...
    summary: {
      totalSpentByCurrency: formattedSummary.map(({ currencyCode, formattedTotalPrice }) => ({
        currencyCode,
        formattedTotalPrice,
      })),
    },
...
const restoreOrder = asyncHandler(async (req, res) => {
  ...
  res.json((await formatOrders([restoredOrder], req.locale))[0]);
});
