...
const getAddresses = asyncHandler(async (req, res) => {
  ...
  if (!customerId) {
    return res.status(400).json({ message: getMessage('VI', 'address.customerRequired') });
  }

  // Verify customer exists
  const customer = await Customer.findOne({ _id: customerId, isDeleted: false });
  if (!customer) {
    return res.status(404).json({ message: getMessage('VI', 'address.customerNotFound') });
  }
...
const getAddressById = asyncHandler(async (req, res) => {
  ...
  if (!address) {
    return res.status(404).json({ message: getMessage('VI', 'address.notFound') });
  }
...
const createAddress = asyncHandler(async (req, res) => {
  ...
  if (!customerId || !fullName || !phone || !provinceId || !districtId || !wardId || !street) {
    return res.status(400).json({ message: getMessage('VI', 'address.missingFields') });
  }
