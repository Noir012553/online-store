...
const getCustomerById = asyncHandler(async (req, res) => {
  ...
  if (!customer) {
    res.status(404);
    throw new Error(getMessage(lang, 'admin-controllers-messages.customer_not_found'));
  }
  ...
  const [totalOrders, totalSpent, currencies] = await Promise.all([
    Order.countDocuments(orderQuery),
    sumOrdersInCurrency(orderQuery, reportingCurrency),
...
const createCustomer = asyncHandler(async (req, res) => {
  ...
  if (customerExists) {
    res.status(400);
    throw new Error(getMessage(lang, 'admin-controllers-messages.customer_already_exists'));
  }
...
const updateCustomer = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (err.code === 11000) {
      res.status(409);
      throw new Error(getMessage(lang, 'admin-controllers-messages.email_already_in_use'));
    }
    throw err;
  }
});
...
const deleteCustomer = asyncHandler(async (req, res) => {
  ...
  if (customer) {
    ...
    res.json({ message: getMessage(lang, 'admin-controllers-messages.customer_removed') });
  } else {
    res.status(404);
    throw new Error(getMessage(lang, 'admin-controllers-messages.customer_not_found'));
  }
...
const restoreCustomer = asyncHandler(async (req, res) => {
  ...
  if (!customer.isDeleted) {
    res.status(400);
    throw new Error(getMessage(lang, 'admin-controllers-messages.customer_not_deleted'));
  }
...
const hardDeleteCustomer = asyncHandler(async (req, res) => {
  ...
  res.json({ message: getMessage(lang, 'admin-controllers-messages.customer_permanently_removed') });
});
...
const createOrUpdateCustomerByPhone = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.json({
      message: updatedCustomer.createdAt === updatedCustomer.updatedAt ? getMessage(lang, 'admin-controllers-messages.customer_created') : getMessage(lang, 'admin-controllers-messages.customer_updated'),
