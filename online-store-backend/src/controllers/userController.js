...
const authUser = asyncHandler(async (req, res) => {
    ...
    if (user && (await user.matchPassword(password))) {
        ...
        res.status(401);
        throw new Error(getMessage('VI', 'auth.invalidEmailPassword'));
    }
});
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if (userExists) {
        res.status(400);
        throw new Error(getMessage('VI', 'user.alreadyExists'));
    }
    ...
    if (user) {
        ...
        res.status(400);
        throw new Error(getMessage('VI', 'user.invalidData'));
    }
});
...
const getUserProfile = asyncHandler(async (req, res) => {
    ...
    if (user) {
        ...
        res.status(404);
        throw new Error(getMessage('VI', 'user.notFound'));
    }
});
...
const hardDeleteUser = asyncHandler(async (req, res) => {
    ...
    res.json({ message: getMessage('VI', 'user.hardDeleteSuccess') });
});
...
const restoreUser = asyncHandler(async (req, res) => {
    ...
    if (!user.isDeleted) {
        res.status(400);
        throw new Error(getMessage('VI', 'user.notDeleted'));
    }

    user.isDeleted = false;
    await user.save();

    res.json({ message: getMessage('VI', 'user.restoreSuccess'), user });
});
...
const uploadUserAvatar = asyncHandler(async (req, res) => {
  ...
  // Path from multer includes destination folder (e.g., uploads/admins/filename)
