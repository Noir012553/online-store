...
const getCategories = asyncHandler(async (req, res) => {
  try {
    ...
    res.json({ categories: categories.map(c => c.toObject ? c.toObject() : c), page, pages: Math.ceil(count / pageSize) });
  } catch (error) {
    throw error;
  }
});
...
const getCategoryById = asyncHandler(async (req, res) => {
  ...
  if (category) {
    res.json(category.toObject ? category.toObject() : category);
  } else {
    res.status(404);
...
const createCategory = asyncHandler(async (req, res) => {
  const { name, description, translationKey, icon, image, key } = req.body;
  ...
  res.status(201).json(createdCategory.toObject ? createdCategory.toObject() : createdCategory);
});
...
const updateCategory = asyncHandler(async (req, res) => {
  ...
  if (category) {
    ...
    res.json(updatedCategory.toObject ? updatedCategory.toObject() : updatedCategory);
  } else {
    res.status(404);
    throw new Error('Category not found');
  }
