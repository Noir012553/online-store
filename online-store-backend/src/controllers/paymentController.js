...
const PAYMENT_MESSAGE_KEYS = {
  ...
  PAYMENT_REQUIRED_FIELDS: 'missing_required_fields',
  PAYMENT_ORDER_ALREADY_PAID: 'order_already_paid',
  PAYMENT_CLIENT_IP_UNAVAILABLE: 'cannot_determine_client_ip',
};
...
const createPaymentError = (result, lang) => {
  const code = result.code || 'PAYMENT_INITIATION_FAILED';
  const error = new Error(getPaymentMessage(lang, code, result.params));
...
const initiatePayment = asyncHandler(async (req, res) => {
  ...
  if (!clientIp || clientIp === '0.0.0.0') {
    res.status(400);
    throw createPaymentError({ code: 'PAYMENT_CLIENT_IP_UNAVAILABLE' }, req.lang);
  }
  ...
  return res.status(200).json({
    success: true,
    data: await formatPaymentResponse(result.data, req.locale),
  });
});
...
const handleWebhook = asyncHandler(async (req, res) => {
  ...
  if (!signature || signature === undefined) {
    ...
    res.status(200).json({
      ...
      message: getPaymentMessage(req.lang, 'PAYMENT_WEBHOOK_SIGNATURE_MISSING', { field: 'signature' }),
    });
    return;
  }
...
const getPaymentHistory = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    data: await formatPayments(result.data, req.locale),
  });
});
...
const refundPayment = asyncHandler(async (req, res) => {
  ...
  if (!payment) {
    res.status(404);
    throw createPaymentError({ code: 'PAYMENT_NOT_FOUND' }, req.lang);
  }
...
const getPaymentDetails = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    data: await formatPaymentResponse(payment, req.locale),
  });
});
...
const getAllPayments = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    data: await formatPayments(payments, req.locale),
    pagination: {
      page: parseInt(page),
      limit: parseInt(limit),
      total,
