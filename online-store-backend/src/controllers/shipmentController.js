...
const createShipmentError = (lang, errorCode, messageKey, params = {}) => {
  const error = new Error(getMessage(lang, messageKey, params));
...
const createShipment = asyncHandler(async (req, res) => {
  ...
  if (!orderId || !shippingProvider || !shippingService) {
    res.status(400);
    throw createShipmentError(req.lang, 'SHIPMENT_REQUIRED_FIELDS', 'shipment.requiredFields');
  }
  ...
  if (order.ghnOrderCode) {
    res.status(400);
    throw createShipmentError(req.lang, 'SHIPMENT_ALREADY_CREATED', 'shipment.alreadyCreated', { orderId });
  }
  ...
  if (!recipientName || !recipientPhone || !recipientAddress || !districtId || !wardCode) {
    res.status(400);
    throw createShipmentError(req.lang, 'SHIPMENT_RECIPIENT_INFO_INVALID', 'shipment.missingInfo');
  }
  ...
  if (!availableServices || availableServices.length === 0) {
    res.status(400);
    throw createShipmentError(req.lang, 'SHIPMENT_SERVICE_UNAVAILABLE', 'shipment.noServiceAvailable', { shippingProvider });
  }
  ...
  if (shippingProvider !== 'ghn') {
    res.status(400);
    throw createShipmentError(req.lang, 'SHIPMENT_PROVIDER_UNSUPPORTED', 'shipment.unsupportedCarrier', { shippingProvider });
  }
  ...
  }, currencies.get(providerCurrencyCode), req.locale, [
    ['totalFee', 'formattedTotalFee'],
  ]);
  const [formattedOrder] = await formatOrders([order], req.locale);
