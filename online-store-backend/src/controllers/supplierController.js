...
const getSupplierById = asyncHandler(async (req, res) => {
  ...
  if (supplier) {
    ...
    res.status(404);
    throw new Error(getMessage(lang, 'admin-controllers-messages.supplier_not_found'));
  }
});
...
const createSupplier = asyncHandler(async (req, res) => {
  ...
  if (supplierExists) {
    res.status(400);
    throw new Error(getMessage(lang, 'admin-controllers-messages.supplier_already_exists'));
  }
...
const deleteSupplier = asyncHandler(async (req, res) => {
  ...
  if (supplier) {
    ...
    await supplier.save();
    res.json({ message: getMessage(lang, 'admin-controllers-messages.supplier_removed') });
  } else {
    res.status(404);
    throw new Error(getMessage(lang, 'admin-controllers-messages.supplier_not_found'));
  }
...
const hardDeleteSupplier = asyncHandler(async (req, res) => {
  ...
  res.json({ message: getMessage(lang, 'admin-controllers-messages.supplier_permanently_removed') });
});
