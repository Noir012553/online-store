...
const reviewSchema = mongoose.Schema(
  {
    ...
    role: (() => {
      ...
      getActiveLangCodes().forEach(lang => {
        localeFields[lang] = { type: String, default: '' };
      });
      return localeFields;
    })(),
    user: {
...
reviewSchema.index({ product: 1, isDeleted: 1 }); // Optimized for getProductReviews + aggregation $match
reviewSchema.index({ user: 1 });
reviewSchema.index({ product: 1, user: 1, isDeleted: 1 }); // Optimized for findOne alreadyReviewed check
