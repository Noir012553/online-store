...
describe('VNPAYAdapter', () => {
  ...
  describe('supported currencies', () => {
    it('should only support VND', () => {
      expect(adapter.supportsCurrency('VND')).to.be.true;
      expect(adapter.supportsCurrency('USD')).to.be.false;
      expect(adapter.supportsCurrency('EUR')).to.be.false;
      expect(adapter.supportsCurrency('SEK')).to.be.false;
    });
  });
  ...
  describe('normalizeAmount', () => {
    ...
    it('should reject non-VND currency', () => {
      expect(() => adapter.normalizeAmount(5000, 'USD')).to.throw('VNPAY only accepts VND amounts');
    });
  });

  describe('provider boundary', () => {
    it('should build a VND provider payload', () => {
      const payload = adapter.buildProviderPayload({
        amount: 1250.5,
        currency: 'VND',
        txnRef: 'ORDER-1-123',
        description: 'Order+1',
        ipAddress: '127.0.0.1',
        createDate: '20240101120000',
      });

      expect(payload.vnp_CurrCode).to.equal('VND');
      expect(payload.vnp_Amount).to.equal('125050');
    });

    it('should normalize a VND provider response', () => {
      const transaction = adapter.normalizeProviderResponse({
        vnp_TxnRef: 'ORDER-1-123',
      ...
      expect(transaction.currency).to.equal('VND');
      expect(transaction.status).to.equal('success');
      expect(transaction.amount).to.equal(1250);
    });
  });
  ...
  describe('verifyChecksum', () => {
    ...
    it('should ignore vnp_SecureHashType during verification', async () => {
      const webhookData = {
        vnp_Amount: '100000',
        vnp_Command: 'pay',
        vnp_TmnCode: mockConfig.partnerId,
      ...
      const signatureData = adapter.createVnpayIpnSignatureString(adapter.sortObject(webhookData));
      const validSignature = adapter.createSignature(signatureData, mockConfig.partnerKey, 'sha512');
