...
function validateProduct(product, rowIndex = 0) {
  ...
  for (const field of REQUIRED_FIELDS) {
    ...
    if (!value) {
      errors.push(`Dòng ${rowIndex}: Thiếu field bắt buộc "${field}"`);
    } else {
      cleaned[field] = value;
    }
  }
  ...
  if (product.price) {
    ...
    if (isNaN(price) || price <= 0) {
      errors.push(`Dòng ${rowIndex}: Price phải là số > 0, nhận được: ${product.price}`);
    } else {
      cleaned.price = price;
    }
  }
  ...
  if (product.originalPrice) {
    ...
    if (isNaN(origPrice) || origPrice <= 0) {
      warnings.push(`Dòng ${rowIndex}: originalPrice không hợp lệ, bỏ qua`);
    } else {
      cleaned.originalPrice = origPrice;
      // Check: originalPrice >= price
      if (origPrice < cleaned.price) {
        warnings.push(`Dòng ${rowIndex}: originalPrice < price, có thể không đúng ý`);
      }
    }
  }
  ...
  if (product.countInStock) {
    ...
    if (isNaN(stock) || stock < 0) {
      warnings.push(`Dòng ${rowIndex}: countInStock không hợp lệ, mặc định = 0`);
  ...
  if (product.features) {
    try {
      ...
      warnings.push(`Dòng ${rowIndex}: Không thể parse features, bỏ qua`);
      cleaned.features = [];
    }
  } else {
    cleaned.features = [];
  ...
  if (product.deal) {
    try {
      ...
      if (typeof product.deal === 'string') {
        ...
        throw new Error('Deal phải là JSON object');
      }
      ...
      if (dealObj && typeof dealObj === 'object') {
        ...
        if (dealObj.discount !== undefined && dealObj.discount !== '' && dealObj.discount !== null) {
          ...
          if (isNaN(discount) || discount < 0 || discount > 100) {
            warnings.push(`Dòng ${rowIndex}: Deal discount phải từ 0-100, nhận được: ${dealObj.discount}`);
          } else {
        ...
        if (dealObj.endTime !== undefined && dealObj.endTime !== '' && dealObj.endTime !== null) {
          ...
          if (isNaN(endTime.getTime())) {
            warnings.push(`Dòng ${rowIndex}: Deal endTime không hợp lệ, phải là valid date`);
          } else if (endTime <= new Date()) {
            warnings.push(`Dòng ${rowIndex}: Deal endTime phải là ngày tương lai, không phải quá khứ`);
          } else {
      ...
      warnings.push(`Dòng ${rowIndex}: Không thể parse deal object, bỏ qua. Error: ${err.message}`);
    }
  }
