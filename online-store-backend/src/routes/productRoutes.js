...
router.route('/')
  .get(getProducts)
  .post(protect, admin, uploadCloudinary.single('image'), createProduct);
...
router.route('/:id')
  .get(getProductById)
  .put(protect, admin, uploadCloudinary.single('image'), updateProduct)
 ...
 */
router.post('/upload', protect, admin, uploadCloudinary.single('image'), asyncHandler(async (req, res) => {
  if (!req.file) {
