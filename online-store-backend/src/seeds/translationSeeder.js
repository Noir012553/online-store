 ...
 * by reading files from the base language (vi) folder
 */
function getAvailableNamespaces() {
  const viLocalePath = path.join(LOCALES_PATH, 'vi');

  if (!fs.existsSync(viLocalePath)) {
    return [];
  }

  try {
    const files = fs.readdirSync(viLocalePath);
