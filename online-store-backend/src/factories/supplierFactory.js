...
class SupplierFactory {
  ...
  static create(overrides = {}, index = 0) {
    ...
    return {
      ...
      sourceNames: overrides.sourceNames || base.sourceNames || [],
      isDefaultImportSupplier: overrides.isDefaultImportSupplier ?? base.isDefaultImportSupplier ?? false,
      phone: overrides.phone || `+84${Math.floor(Math.random() * 900000000 + 100000000)}`,
      address: overrides.address || `${(index % 50) + 1} Business Street, Hanoi, Vietnam`,
    };
  }
   ...
   * @param {Object} overrides - Dữ liệu override
   */
  static createMany(count = suppliers.length, overrides = {}) {
    const result = [];
