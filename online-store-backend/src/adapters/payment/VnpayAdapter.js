...
class VnpayAdapter extends BasePaymentGateway {
  ...
  async createPaymentUrl(paymentData) {
    try {
      ...
      const vnpayDataForHash = this.buildProviderPayload({
        amount,
        currency,
        txnRef,
      ...
      console.log(`   TxnRef: ${sortedData.vnp_TxnRef}`);
      console.log(`   ReturnUrl: ${sortedData.vnp_ReturnUrl}`);
      console.log(`   OrderInfo: ${sortedData.vnp_OrderInfo}`);
  ...
  async verifyChecksum(webhookData, signature) {
    try {
      ...
      const rawSignatureData = this.formatDataForSignature(sortedData, 'query');
      const receivedSignature = String(signature || vnp_SecureHash).toLowerCase();
      const expectedSignatures = [signatureData, rawSignatureData]
        .map(data => this.createSignature(data, this.config.partnerKey, 'sha512').toLowerCase());

      const crypto = require('crypto');
      const isValidSignature = expectedSignatures.some(expectedSignature =>
  ...
  async handleIPN(ipnData) {
    try {
      ...
      // ============================================
      const responseCurrency = ipnData.vnp_CurrCode
        ? String(ipnData.vnp_CurrCode).toUpperCase()
        : 'VND';

      if (responseCurrency !== 'VND') {
        return {
          success: false,
          error: 'VNPAY response currency must be VND',
        };
      }

      const transaction = this.normalizeProviderResponse(ipnData, {
        responseCode,
        transactionStatus,
        amount,
        orderId,
  ...
  buildProviderPayload({ amount, currency, txnRef, description, ipAddress, createDate }) {
    return {
      vnp_Version: '2.1.0',
      vnp_Command: 'pay',
      vnp_TmnCode: this.config.partnerId,
      vnp_Amount: String(this.normalizeAmount(amount, currency)),
      vnp_CurrCode: 'VND',
      vnp_TxnRef: txnRef,
  ...
  normalizeProviderResponse(ipnData, { responseCode, transactionStatus, amount, orderId }) {
    return {
  ...
  formatDate(date) {
    ...
    // Note: 'en-US' locale is required by VNPAY API, not arbitrary preference
